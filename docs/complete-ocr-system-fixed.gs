...
function setupPushNotifications() {
  try {
    console.log('=== 自動OCR設定開始 ===');
    
    // 既存のトリガーを削除
    const triggers = ScriptApp.getProjectTriggers();
    triggers.forEach(trigger => {
      if (trigger.getHandlerFunction() === 'checkAndProcessRecentFiles') {
    ...
    console.log('✅ 自動OCR設定完了！');
    console.log('📋 5分ごとに新しいPDFファイルをチェックしてOCR処理を実行します');
    console.log('🎯 Google Driveの監視フォルダにPDFをアップロードするだけでOKです');
    
    // 初回実行
    console.log('初回チェックを実行中...');
...
function removeAutoOCR() {
  try {
    ...
    if (removed > 0) {
      console.log(`✅ ${removed}個の自動OCRトリガーを削除しました`);
    } else {
      console.log('⚠️ 自動OCRトリガーは設定されていません');
    }
...
function checkTriggers() {
  ...
  if (triggers.length === 0) {
    console.log('⚠️ トリガーは設定されていません');
  } else {
    console.log(`📋 ${triggers.length}個のトリガーが設定されています:`);
