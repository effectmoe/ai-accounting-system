...
export default function EditDocumentMongoDB() {
  ...
  const fetchDocument = async () => {
    try {
      const response = await fetch(`/api/documents/list?documentId=${documentId}`);
        ...
        <div className="bg-white rounded-lg shadow p-6 space-y-6">
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                type="date"
                value={document.receipt_date || document.issue_date?.split('T')[0] || ''}
                className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-100"
                disabled
              />
            </div>
          ...
          <div>
            ...
            <input
              type="text"
              value={document.vendor_name || document.partner_name || ''}
          ...
          {document.document_type !== 'journal_entry' && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                勘定科目
              </label>
              <select
                value={document.category || '未分類'}
