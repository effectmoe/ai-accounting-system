...
function ProductsPageContent() {
  ...
  const [sortBy, setSortBy] = useState<ProductSortableField>('createdAt');
      ...
      render: (product) => (
        <div>
          ...
          {product.productNameKana && (
            <div className="text-xs text-gray-500">{product.productNameKana}</div>
          )}
        </div>
      ),
    },
  ...
  useEffect(() => {
    ...
    const sortByParam = searchParams.get('sortBy') as ProductSortableField;
  ...
  // ソート処理
  const handleSort = (field: ProductSortableField) => {
    if (sortBy === field) {
      // 同じフィールドの場合は順序を切り替え
  ...
  // ソートアイコンコンポーネント
  const SortIcon = ({ field }: { field: ProductSortableField }) => {
    if (sortBy !== field) {
  ...
  const handleStatusUpdate = async (productId: string, newStatus: boolean) => {
    ...
    try {
      ...
      // 成功時のトースト表示
      toast.success(`ステータスを${newStatus ? 'アクティブ' : '非アクティブ'}に更新しました`);
  ...
  const deleteSelectedProducts = async () => {
    ...
    try {
      ...
      if (failedDeletions > 0) {
        ...
        toast.success(`${selectedProducts.size}件の商品を削除しました`);
      }
  ...
  // 一括ステータス変更
  const updateSelectedProductsStatus = async (newStatus: boolean) => {
    if (selectedProducts.size === 0) return;
    ...
    try {
      ...
      if (failedUpdates > 0) {
        toast.error(`${failedUpdates}件の更新に失敗しました`);
      } else {
        toast.success(`${selectedProducts.size}件の商品を${newStatus ? 'アクティブ' : '非アクティブ'}にしました`);
      }
  ...
  // フィルター処理（デバウンス付き）
  const handleFilterChange = useCallback((newFilters: Partial<ProductFilterState>) => {
      ...
      <div className="mb-6 bg-white rounded-lg shadow p-4">
                ...
                <div className="space-y-2">
                  {[0, 0.08, 0.1].map((rate) => (
                    <label key={rate} className="flex items-center">
                      <input
                        type="checkbox"
                        checked={filters.taxRates?.includes(rate) || false}
                        onChange={(e) => {
        ...
        {getActiveFilterCount() > 0 && (
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {filters.taxRates && filters.taxRates.length > 0 && (
              <div className="inline-flex items-center gap-1 bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-sm">
                税率: {filters.taxRates.map(r => `${(r * 100).toFixed(0)}%`).join(', ')}
                <button
                  onClick={() => removeFilter('taxRates')}
                  className="ml-1 hover:bg-blue-200 rounded-full p-0.5"
                >
                  <X className="w-3 h-3" />
                </button>
            ...
            {(filters.stockQuantityMin !== undefined || filters.stockQuantityMax !== undefined) && (
              <div className="inline-flex items-center gap-1 bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-sm">
                在庫数: 
              ...
              <tr>
                ...
                {visibleColumns.map((column) => (
                  <th key={column.key} className="p-4 text-left text-sm font-medium text-gray-700">
                    {column.sortable ? (
                      <button
                        onClick={() => handleSort(column.key as ProductSortableField)}
                        className="flex items-center gap-1 hover:text-blue-600 transition-colors"
                      >
                        {column.label}
                        <SortIcon field={column.key as ProductSortableField} />
                      </button>
                    ) : (
                    ...
                    <tr key={productId} className="border-b hover:bg-gray-50">
                      ...
                      {visibleColumns.map((column) => (
                        <td key={column.key} className={`p-4 text-sm ${
                          column.key === 'productName' ? 'font-medium' : 'text-gray-600'
                        }`}>
                      ...
                      <td className="p-4 text-sm">
                        <button
                          onClick={() => handleStatusUpdate(productId, !product.isActive)}
                          disabled={updatingStatus.has(productId)}
