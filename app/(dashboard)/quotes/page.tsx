...
function QuotesPageContent() {
  ...
  const [undoingQuotes, setUndoingQuotes] = useState<Set<string>>(new Set());
  ...
  const handleUndoConvertToInvoice = async (quoteId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    setUndoingQuotes(prev => new Set(prev).add(quoteId));

    try {
      const response = await fetch(`/api/quotes/${quoteId}/undo-convert-to-invoice`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (typeof window !== 'undefined' && (window as any).Sentry) {
        (window as any).Sentry.captureException(error, {
          tags: {
            operation: 'undo_quote_to_invoice_conversion',
            quoteId: quoteId,
          },
          extra: {
            errorMessage: error instanceof Error ? error.message : String(error),
                    ...
                    >
                      ...
                      <TableCell onClick={(e) => e.stopPropagation()}>
                        <div className="flex gap-1">
                          ...
                          {quote.convertedToInvoiceId && quote.status === 'converted' && (
                            <>
                              <Button
                                variant="ghost"
                                size="sm"
                              ...
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={(e) => handleUndoConvertToInvoice(quote._id, e)}
                                disabled={undoingQuotes.has(quote._id)}
                                title="変換を取り消し"
                                className="hover:bg-red-50 hover:text-red-600 transition-colors duration-75 group"
                              >
                                {undoingQuotes.has(quote._id) ? (
                                  <Loader2 className="h-4 w-4 animate-spin" />
                                ) : (
                                  <Undo2 className="h-4 w-4" />
                                )}
                                <span className="sr-only">変換を取り消し</span>
                              </Button>
                            </>
                          )}
                          <Button
