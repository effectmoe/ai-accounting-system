import { JournalEntry, JournalLine } from '@/types/collections';
 ...
 * @returns 借方合計金額
 */
export function calculateDebitTotal(lines: JournalLine[]): number {
 ...
 * @returns 貸方合計金額
 */
export function calculateCreditTotal(lines: JournalLine[]): number {
 ...
 * @returns 貸借差額の絶対値
 */
export function calculateBalanceDifference(journal: JournalEntry): number {
  const debitTotal = calculateDebitTotal(journal.lines);
  const creditTotal = calculateCreditTotal(journal.lines);
