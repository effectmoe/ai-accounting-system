...
export class FreeeImporter {
      ...
      try {
        ...
        if (!dryRun) {
          try {
            ...
            await vercelDb.updateOne(
              'customers',
              { company_id: companyId, customer_code: customerCode },
              { $set: customerData },
              { upsert: true }
            );
          } catch (error) {
      ...
      try {
        ...
        if (!dryRun) {
          try {
            ...
            await vercelDb.updateOne(
              'accounts',
              { company_id: companyId, code: accountCode },
              { $set: accountData },
              { upsert: true }
            );
          } catch (error) {
  ...
  private async importTransactions(records: any[], companyId: string, dryRun: boolean) {
    ...
    for (const record of records) {
      try {
        ...
        if (!dryRun) {
          // Insert transaction
          const transaction = await vercelDb.create('transactions', transactionData);
          if (!transaction || !transaction._id) throw new Error('Failed to create transaction');
          
          // Get account ID
          const account = await vercelDb.findOne('accounts', {
            company_id: companyId,
            name: validated['勘定科目']
          });
          ...
          if (!account) {
            ...
            const lineResult = await vercelDb.create('transaction_lines', lineData);
  ...
  private async generateTransactionNumber(companyId: string, date: string): Promise<string> {
    ...
    const count = await vercelDb.count('transactions', {
      company_id: companyId,
      ...
      try {
        ...
        if (!dryRun) {
          try {
            await vercelDb.updateOne(
              'items',
              { company_id: companyId, name: validated['品目'] },
              { $set: itemData },
              { upsert: true }
            );
          } catch (error) {
      ...
      try {
        ...
        if (!dryRun) {
          try {
            await vercelDb.updateOne(
              'memo_tags',
              { company_id: companyId, name: validated['メモタグ'] },
              { $set: tagData },
              { upsert: true }
            );
          } catch (error) {
      ...
      try {
        ...
        if (!dryRun) {
          try {
            await vercelDb.updateOne(
              'departments',
              { company_id: companyId, code: departmentData.code },
              { $set: departmentData },
              { upsert: true }
            );
          } catch (error) {
