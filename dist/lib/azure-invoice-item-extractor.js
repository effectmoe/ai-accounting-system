...
exports.InvoiceItemExtractor = void 0;
const logger_1 = require("@/lib/logger");
/**
 * 日本の請求書・見積書から商品明細を抽出するためのヘルパークラス
 */
class InvoiceItemExtractor {
    /**
     * Azure Form Recognizerの結果から商品明細を抽出（強化版）
     */
    static extractItems(analysisResult) {
        const items = [];
        logger_1.logger.debug('[InvoiceItemExtractor] 商品抽出開始:', {
        ...
        if (analysisResult.fields?.items && Array.isArray(analysisResult.fields.items)) {
            logger_1.logger.debug('[InvoiceItemExtractor] 標準itemsフィールドから抽出');
        ...
        if (analysisResult.rawResult) {
            ...
            if (rawItems.length > 0) {
                logger_1.logger.debug('[InvoiceItemExtractor] rawResultから商品明細を抽出:', rawItems);
                return rawItems;
            }
        }
        ...
        if (analysisResult.fields?.customFields) {
            ...
            if (customItems.length > 0) {
                logger_1.logger.debug('[InvoiceItemExtractor] customFieldsから商品明細を抽出:', customItems);
                return customItems;
            }
        }
        ...
        if (analysisResult.pages && analysisResult.pages.length > 0) {
            ...
            if (pageItems.length > 0) {
                logger_1.logger.debug('[InvoiceItemExtractor] ページコンテンツから商品明細を抽出:', pageItems);
                return pageItems;
            }
        }
        // 5. テーブルから抽出（既存のロジック）
        if (analysisResult.tables && analysisResult.tables.length > 0) {
            ...
            if (tableItems.length > 0) {
                logger_1.logger.debug('[InvoiceItemExtractor] テーブルから商品明細を抽出:', tableItems);
                return tableItems;
            }
        }
        // 商品情報が全く見つからない場合は空配列を返す
        logger_1.logger.warn('[InvoiceItemExtractor] 商品明細を抽出できませんでした');
        return [];
    }
    /**
     * rawResultから商品明細を抽出
     */
    static extractItemsFromRawResult(rawResult) {
        const items = [];
        try {
            // documentsフィールドをチェック
            ...
            logger_1.logger.error('[InvoiceItemExtractor] rawResult解析エラー:', error);
        }
        return items;
    }
    /**
     ...
     */
    static extractItemsFromCustomFields(customFields) {
        const items = [];
        try {
            ...
            logger_1.logger.error('[InvoiceItemExtractor] customFields解析エラー:', error);
        }
        return items;
    }
    /**
    ...
    static extractItemsFromPages(pages) {
        const items = [];
        try {
            logger_1.logger.debug('[InvoiceItemExtractor] ページ数:', pages.length);
            for (const page of pages) {
                if (page.lines && Array.isArray(page.lines)) {
                    logger_1.logger.debug('[InvoiceItemExtractor] ページの行数:', page.lines.length);
                    ...
                    page.lines.forEach((line, index) => {
                        ...
                        if (content.includes('【') || content.includes('用紙') ||
                            ...
                            logger_1.logger.debug(`[InvoiceItemExtractor] 行[${index}]:`, content);
                        }
                    });
            ...
            logger_1.logger.error('[InvoiceItemExtractor] ページ解析エラー:', error);
        }
        return items;
    }
    /**
     ...
     */
    static extractItemsFromText(text) {
        const items = [];
        try {
                ...
                for (const match of matches) {
                    ...
                    if (itemName &&
                        ...
                        if (item.amount > 0 || item.unitPrice > 0) {
                            items.push(item);
                            logger_1.logger.debug('[InvoiceItemExtractor] テキストから商品を抽出:', item);
                        }
                    }
                }
            }
            ...
            for (const pattern of specificPatterns) {
                ...
                for (const match of matches) {
                    ...
                    if (itemName && !items.some(item => item.description.includes(itemName))) {
                        ...
                        logger_1.logger.debug('[InvoiceItemExtractor] 特定パターンから商品を抽出:', itemName);
                    }
                }
            }
        }
        catch (error) {
            logger_1.logger.error('[InvoiceItemExtractor] テキスト解析エラー:', error);
        }
        return items;
    }
    /**
    ...
    static extractItemsFromTables(tables) {
        ...
        try {
            ...
            logger_1.logger.error('[InvoiceItemExtractor] テーブル解析エラー:', error);
        }
        return items;
    }
    /**
     * テーブル構造を解析
     */
    static analyzeTableStructure(table) {
        const structure = {
     ...
     * テーブル行から商品情報を抽出
     */
    static extractItemFromTableRow(cells, structure) {
        const item = {
            description: '',
            name: '',
            quantity: 1,
     ...
     */
    static extractItemFromFields(fields) {
        const item = {
            description: '',
            name: '',
            quantity: 1,
     ...
     * 内容が数値のみかチェック
     */
    static isNumericContent(content) {
        const cleaned = content.replace(/[¥￥$,\s円]/g, '');
        return /^\d+(?:\.\d+)?$/.test(cleaned);
    }
}
exports.InvoiceItemExtractor = InvoiceItemExtractor;
