...
class FreeeImporter {
    constructor(supabaseUrl, supabaseKey) {
        ...
        if (supabaseUrl || supabaseKey) {
            logger_1.logger.warn('FreeeImporter: supabaseUrl and supabaseKey parameters are deprecated. Using MongoDB.');
        }
    }
    async importFile(filePath, options) {
        const { fileType, encoding = 'sjis', dryRun = false, companyId } = options;
        logger_1.logger.debug(`[Freee Import] Starting import of ${fileType} from ${filePath}`);
        ...
        if (records.length > 0) {
            logger_1.logger.debug('[Freee Import] CSV headers:', Object.keys(records[0]));
            logger_1.logger.debug('[Freee Import] First record:', records[0]);
            // Additional debug for partner import
            if (fileType === 'partners') {
                logger_1.logger.debug('[Freee Import] Debug - Looking for field "名前（通称）"');
                logger_1.logger.debug('[Freee Import] Debug - Available fields:', Object.keys(records[0]).map(k => `"${k}"`).join(', '));
            }
        }
        logger_1.logger.debug(`[Freee Import] Found ${records.length} records`);
        switch (fileType) {
            case 'partners':
    ...
    async importPartners(records, companyId, dryRun) {
        ...
        for (const record of records) {
            try {
                ...
                logger_1.logger.debug(`[Freee Import] Imported partner: ${customerData.name}`);
                results.success++;
            }
            catch (error) {
                logger_1.logger.error(`[Freee Import] Failed to import partner:`, error);
                results.failed++;
    ...
    async importAccounts(records, companyId, dryRun) {
        ...
        for (const record of records) {
            try {
                ...
                logger_1.logger.debug(`[Freee Import] Imported account: ${accountData.name}`);
                results.success++;
            }
            catch (error) {
                logger_1.logger.error(`[Freee Import] Failed to import account:`, error);
                results.failed++;
                results.errors.push({ record, error });
            }
        }
        ...
        for (const record of records) {
            try {
                ...
                if (!dryRun) {
                    ...
                    if (!account) {
                        logger_1.logger.warn(`Account not found: ${validated['勘定科目']}`);
                    }
                    else {
                ...
                logger_1.logger.debug(`[Freee Import] Imported transaction: ${transactionData.description}`);
                results.success++;
            }
            catch (error) {
                logger_1.logger.error(`[Freee Import] Failed to import transaction:`, error);
                results.failed++;
                results.errors.push({ record, error });
            }
        }
    ...
    async importItems(records, companyId, dryRun) {
        ...
        for (const record of records) {
            try {
                ...
                logger_1.logger.debug(`[Freee Import] Imported item: ${itemData.name}`);
                results.success++;
            }
            catch (error) {
                logger_1.logger.error(`[Freee Import] Failed to import item:`, error);
                results.failed++;
                results.errors.push({ record, error });
            }
        }
    ...
    async importMemoTags(records, companyId, dryRun) {
        ...
        for (const record of records) {
            try {
                ...
                logger_1.logger.debug(`[Freee Import] Imported memo tag: ${tagData.name}`);
                results.success++;
            }
            catch (error) {
                logger_1.logger.error(`[Freee Import] Failed to import memo tag:`, error);
                results.failed++;
                results.errors.push({ record, error });
            }
        }
    ...
    async importDepartments(records, companyId, dryRun) {
        ...
        for (const record of records) {
            try {
                ...
                logger_1.logger.debug(`[Freee Import] Imported department: ${departmentData.name}`);
                results.success++;
            }
            catch (error) {
                logger_1.logger.error(`[Freee Import] Failed to import department:`, error);
                results.failed++;
                results.errors.push({ record, error });
            }
        }
