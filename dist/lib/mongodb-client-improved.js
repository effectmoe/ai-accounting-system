...
async function isConnectionHealthy() {
    ...
    if (now - connectionState.lastPingTime > 30000) {
        try {
            ...
            logger_1.logger.error('MongoDB ping failed:', error);
            return false;
        }
    }
    return true;
...
async function connectToDatabase() {
    ...
    if (await isConnectionHealthy()) {
        logger_1.logger.debug('Reusing healthy MongoDB connection');
        return { client: connectionState.client, db: connectionState.db };
    }
    // 他のリクエストが接続中の場合は待機
    if (connectionState.isConnecting) {
        logger_1.logger.debug('Waiting for ongoing connection attempt...');
        let waitCount = 0;
    ...
    try {
        ...
        if (connectionState.client) {
            logger_1.logger.debug('Closing stale MongoDB connection');
            try {
                await connectionState.client.close();
            }
            catch (error) {
                logger_1.logger.error('Error closing MongoDB client:', error);
            }
        ...
        // 接続を確認
        await db.admin().ping();
        logger_1.logger.debug('MongoDB connection established successfully');
        // 接続状態を更新
...
if (process.env.VERCEL) {
    ...
    process.on('SIGTERM', async () => {
        logger_1.logger.debug('SIGTERM received, closing MongoDB connection');
