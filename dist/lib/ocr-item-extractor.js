...
class OCRItemExtractor {
    ...
    static extractItemsFromOCR(ocrData) {
        ...
        if (ocrData.tables && Array.isArray(ocrData.tables) && ocrData.tables.length > 0) {
            logger_1.logger.debug('[OCRItemExtractor] tablesデータから商品情報を抽出します');
            const tableItems = this.extractItemsFromTables(ocrData.tables);
            if (tableItems.length > 0) {
                logger_1.logger.debug(`[OCRItemExtractor] テーブルから${tableItems.length}個の商品を抽出しました`);
                return tableItems;
            }
        }
        // 1. 次にpagesデータから抽出（行ベースのデータ）
        if (ocrData.pages && Array.isArray(ocrData.pages) && ocrData.pages.length > 0) {
            logger_1.logger.debug('[OCRItemExtractor] pagesデータから商品情報を抽出します');
            const pageItems = this.extractItemsFromPages(ocrData.pages);
            if (pageItems.length > 0) {
                logger_1.logger.debug(`[OCRItemExtractor] ページから${pageItems.length}個の商品を抽出しました`);
                return pageItems;
            }
        }
        ...
        if (items.length === 0 || items.every(item => this.isDefaultItemName(item.itemName))) {
            logger_1.logger.debug('[OCRItemExtractor] itemsフィールドが不完全。OCR全体から商品情報を抽出します');
            // OCRデータ全体を文字列として検索
    ...
    static extractItemsFromTables(tables) {
        ...
        for (const table of tables) {
            logger_1.logger.debug(`[OCRItemExtractor] テーブル: ${table.rowCount}行 x ${table.columnCount}列`);
            ...
            for (let rowIndex = 1; rowIndex < rows.length; rowIndex++) {
                ...
                if (!hasNumericData || (quantity === 0 && unitPrice === 0 && amount === 0)) {
                    logger_1.logger.debug(`[OCRItemExtractor] 備考として認識: "${productName}"`);
                ...
                if (productName && !this.isDefaultItemName(productName) && (amount > 0 || unitPrice > 0)) {
                    ...
                    logger_1.logger.debug(`[OCRItemExtractor] テーブルから商品抽出: ${productName}, 数量: ${quantity}, 単価: ${unitPrice}, 金額: ${amount}`);
                }
            }
        }
        // 備考をnotesとして結合
        if (remarks.length > 0 && items.length > 0) {
            ...
            logger_1.logger.debug(`[OCRItemExtractor] 備考を追加: ${combinedRemarks}`);
        }
        return items;
    }
    /**
    ...
    static extractItemsFromPages(pages) {
        ...
        for (const page of pages) {
            if (!page.lines || page.lines.length === 0)
                continue;
            logger_1.logger.debug(`[OCRItemExtractor] ページの行数: ${page.lines.length}`);
                ...
                if (amountMatch) {
                    ...
                    if (amount > 1000) { // 1000円以上を商品として認識
                        ...
                        if (productName && !this.isDefaultItemName(productName)) {
                            ...
                            logger_1.logger.debug(`[OCRItemExtractor] ページから商品抽出: ${productName}, 金額: ${amount}`);
                        }
                    }
                }
            }
    ...
    static extractFullText(ocrData) {
                ...
                for (const field of importantFields) {
                    if (obj[field] && !processedKeys.has(`${path}.${field}`)) {
                        ...
                        if (typeof obj[field] === 'string') {
                            ...
                            logger_1.logger.debug(`[OCRItemExtractor] 重要フィールド発見 ${field}:`, obj[field]);
                        }
                        else if (obj[field]?.content || obj[field]?.value) {
                            const value = obj[field].content || obj[field].value;
                            texts.push(String(value));
                            logger_1.logger.debug(`[OCRItemExtractor] 重要フィールド発見 ${field}:`, value);
                        }
                    }
                }
                // その他のフィールドも探索
        ...
        logger_1.logger.debug('[OCRItemExtractor] 抽出されたテキスト総数:', texts.length);
        return fullText;
    }
    /**
     * フルテキストから商品情報を抽出
    ...
    static extractItemsFromFullText(fullText, ocrData) {
        const items = [];
        logger_1.logger.debug('[OCRItemExtractor] フルテキスト:', fullText);
        logger_1.logger.debug('[OCRItemExtractor] OCRデータ構造:', JSON.stringify(ocrData, null, 2));
        ...
        for (const pattern of productPatterns) {
            ...
            logger_1.logger.debug(`[OCRItemExtractor] パターン ${pattern} の結果:`, matches);
            if (matches) {
                for (const match of matches) {
                    ...
                    if (cleaned.length > bestMatchLength && cleaned.length > 5) {
                        ...
                        logger_1.logger.debug(`[OCRItemExtractor] より良い商品名を発見: "${bestMatch}" (長さ: ${bestMatchLength})`);
                    }
                }
            }
        }
