    ...
    for (const [name, config] of Object.entries(exports.mcpServers)) {
        try {
            ...
            if (required) {
                ...
                if (missing.length > 0) {
                    logger_1.logger.warn(`Skipping ${name} MCP server: Missing environment variables: ${missing.join(', ')}`);
                    continue;
                }
            }
            ...
            logger_1.logger.error(`Failed to initialize ${name} MCP server:`, error);
        }
    }
}
