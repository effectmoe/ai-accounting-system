...
async function generateSimplePDF(data) {
    logger_1.logger.debug('=== VERCEL HTML PDF GENERATION START ===');
    logger_1.logger.debug('Document:', data.documentNumber);
    logger_1.logger.debug('Customer name:', data.customerName);
    logger_1.logger.debug('Company info:', data.companyInfo);
    try {
...
<body>
    ...
    `;
        ...
        logger_1.logger.debug('Generated HTML content, converting to PDF...');
        // 詳細なPDFコンテンツを作成
        const itemsText = (data.items || []).map((item, index) => `(${index + 1}. ${item.description || 'Item'} - Qty: ${item.quantity || 1} - Price: ¥${(item.unitPrice || 0).toLocaleString()} - Total: ¥${(item.amount || 0).toLocaleString()})`).join('\n');
        // 安全にテキストをエンコード
...
%%EOF`;
        ...
        logger_1.logger.debug('PDF generated successfully, base64 length:', base64PDF.length);
        return base64PDF;
    }
    catch (error) {
        logger_1.logger.error('Vercel PDF generation error:', error);
        throw error;
    }
}
