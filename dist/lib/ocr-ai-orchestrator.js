...
class OCRAIOrchestrator {
    ...
    async orchestrateOCRResult(request) {
        ...
        try {
            ...
            if (request.documentType === 'receipt') {
                ...
                logger_1.logger.debug('[OCRAIOrchestrator] Receipt type detection:', {
                    documentType: request.documentType,
    ...
    validateAndEnhanceData(data, ocrResult) {
        ...
        if (this.isParkingReceiptFromOCR(JSON.stringify(ocrResult))) {
            logger_1.logger.debug('[OCRAIOrchestrator] Detected parking receipt, enhancing parking fields...');
            // receiptTypeを設定
            ...
            logger_1.logger.debug('[OCRAIOrchestrator] Enhanced parking fields:', {
...
正確な日本語の商習慣を理解して解析してください。`;
     ...
     */
    isParkingReceiptFromOCR(ocrData) {
