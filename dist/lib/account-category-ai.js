...
exports.AccountCategoryAI = void 0;
const mcp_client_1 = require("./mcp-client");
const problem_solving_agent_1 = require("@/agents/problem-solving-agent");
const logger_1 = require("@/lib/logger");
/**
 * AIベースの勘定科目分類システム
 * MCPツール（Perplexity、Firecrawl等）を活用してOCRテキストから勘定科目を高精度で推論
 */
class AccountCategoryAI {
    mcpClient;
    constructor() {
        this.mcpClient = (0, mcp_client_1.getMcpClient)();
    }
    /**
     * OCR結果から勘定科目を推論
     * MCPツールを活用して高精度な分析を実行
     */
    async predictAccountCategory(ocrResult, companyId) {
        try {
            ...
            logger_1.logger.error('AI prediction error:', error);
            // エラーの場合は高度なフォールバック
     ...
     * 業種別パターンと購入品目を詳細に分析
     */
    async extractInformationFromOCR(ocrResult) {
        // textフィールドの安全な処理
     ...
     * 検索クエリを構築
     */
    buildSearchQuery(ocrResult, extractedInfo) {
        const components = [
            ocrResult.vendor || '',
     ...
     * 駐車場レシートかどうかを判定
     */
    isParkingReceipt(ocrResult, extractedInfo) {
     ...
     * 駐車場レシートの高度な分析
     */
    async analyzeParkingReceipt(ocrResult, extractedInfo, accountingInfo) {
     ...
     * 業種と文脈を考慮した高度な判定
     */
    async performComplexAnalysis(ocrResult, extractedInfo, accountingInfo) {
        try {
            // 業種別の判定ロジックを適用
            if (extractedInfo.businessType) {
                return await this.analyzeByBusinessType(ocrResult, extractedInfo, accountingInfo);
            }
          ...
          ${accountingInfo}
        `;
                const solution = await problem_solving_agent_1.problemSolvingAgent.solve(problemDescription, {
                    type: 'accounting',
                    country: 'Japan',
                ...
                logger_1.logger.warn('Problem Solving Agent not available, falling back to rule-based analysis:', agentError);
                // Problem Solving Agentが利用できない場合は、直接フォールバック分析を実行
                return this.intelligentFallback(ocrResult);
            }
        }
        catch (error) {
            logger_1.logger.error('Complex analysis failed:', error);
            // フォールバック
            return this.intelligentFallback(ocrResult);
        }
    }
     ...
     */
    parseSolutionToPrediction(solution, ocrResult) {
        // AIの分析結果から勘定科目を抽出
        ...
        if (!solution.success) {
            logger_1.logger.debug('Problem Solving Agent failed, using fallback logic');
            return this.intelligentFallback(ocrResult);
        }
        // 分析結果から勘定科目を推定
        ...
        Object.entries(categoryMappings).forEach(([category, keywords]) => {
            const matchCount = keywords.filter(kw => analysisText.toLowerCase().includes(kw)).length;
            if (matchCount > 0) {
     ...
     * 業種タイプに基づく詳細分析
     */
    async analyzeByBusinessType(ocrResult, extractedInfo, accountingInfo) {
     ...
     * 飲食店レシートの分析
     */
    analyzeFoodServiceReceipt(ocrResult, extractedInfo, businessType) {
        const amount = ocrResult.amount || 0;
     ...
     * 小売店レシートの分析
     */
    analyzeRetailReceipt(ocrResult, extractedInfo, businessType) {
     ...
     * サービス業レシートの分析
     */
    analyzeServiceReceipt(ocrResult, extractedInfo, businessType) {
        // ガソリンスタンドの場合
     ...
     * 交通機関レシートの分析
     */
    analyzeTransportReceipt(ocrResult, extractedInfo, businessType) {
        // すべての交通機関は基本的に旅費交通費
        return {
            category: '旅費交通費',
            confidence: 0.95,
     ...
     */
    async improveWithLearningData(prediction, companyId, vendorName) {
        try {
            // 学習システムから過去の分類データを取得
            const { AccountLearningSystem } = await Promise.resolve().then(() => __importStar(require('./account-learning-system')));
            const learningSystem = new AccountLearningSystem();
            const learnedPrediction = await learningSystem.predictAccountCategory(companyId, vendorName);
            ...
            logger_1.logger.debug('Learning data not available, using AI prediction only');
        }
        return prediction;
    }
}
exports.AccountCategoryAI = AccountCategoryAI;
