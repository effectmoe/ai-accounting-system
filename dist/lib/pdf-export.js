...
async function downloadPDF(data) {
    try {
        ...
        logger_1.logger.error('PDF生成エラー:', error);
        throw new Error('PDFの生成に失敗しました');
    }
}
// Base64エンコードされたPDFを生成（メール送信などに使用）
async function generatePDFBase64(data) {
    logger_1.logger.debug('generatePDFBase64 called, window check:', typeof window === 'undefined');
    // サーバーサイドで実行される場合
    if (typeof window === 'undefined') {
        logger_1.logger.debug('Running on server side, attempting PDF generation...');
