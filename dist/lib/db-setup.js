...
async function initializeDatabase() {
    try {
        logger_1.logger.debug('Initializing database collections and indexes...');
        const database = await (0, mongodb_client_1.getDatabase)();
        // 1. customersコレクションのインデックス作成
        logger_1.logger.debug('Creating indexes for customers collection...');
        ...
        logger_1.logger.debug('Creating indexes for companyInfo collection...');
        ...
        logger_1.logger.debug('Creating indexes for bankAccounts collection...');
        ...
        logger_1.logger.debug('Creating indexes for invoices collection...');
        ...
        logger_1.logger.debug('Database initialization completed successfully!');
    }
    catch (error) {
...
async function createSampleData() {
    try {
        logger_1.logger.debug('Creating sample data...');
        // 1. 自社情報のサンプル作成
        ...
        if (!existingCompanyInfo) {
            ...
            logger_1.logger.debug('Created sample company info');
        }
        else {
            companyInfoId = existingCompanyInfo._id.toString();
            logger_1.logger.debug('Using existing company info');
        }
        // 2. 銀行口座のサンプル作成
        ...
        if (!existingBankAccount) {
            ...
            logger_1.logger.debug('Created sample bank account');
        }
        else {
            bankAccountId = existingBankAccount._id.toString();
            logger_1.logger.debug('Using existing bank account');
        }
        // 3. 顧客のサンプル作成
        ...
        logger_1.logger.debug('Created sample customer');
        // 4. 請求書のサンプル作成
        ...
        logger_1.logger.debug('Created sample invoice');
        logger_1.logger.debug('Sample data creation completed!');
    }
    catch (error) {
        logger_1.logger.error('Error creating sample data:', error);
        throw error;
    }
}
/**
...
async function setupValidationRules() {
    try {
        logger_1.logger.debug('Setting up database validation rules...');
        const database = await (0, mongodb_client_1.getDatabase)();
        ...
        logger_1.logger.debug('Validation rules setup completed!');
    }
    catch (error) {
        // バリデーションルールの設定はオプショナルなので、エラーは警告として扱う
        logger_1.logger.warn('Warning: Could not set up validation rules:', error);
    }
}
