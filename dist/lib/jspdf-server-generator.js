...
exports.generateJsPDFDocument = generateJsPDFDocument;
const logger_1 = require("@/lib/logger");
// サーバーサイドでjsPDFを使用するための設定
async function generateJsPDFDocument(data) {
    logger_1.logger.debug('=== jsPDF Server Generation START ===');
    logger_1.logger.debug('Document:', data.documentNumber);
    try {
        // jsPDFの動的インポート（サーバーサイド用）
        const { jsPDF } = await Promise.resolve().then(() => __importStar(require('jspdf')));
        // A4サイズのPDFを作成
        const doc = new jsPDF({
        ...
        logger_1.logger.debug('jsPDF generated successfully, size:', pdfBase64.length);
        return pdfBase64;
    }
    catch (error) {
        logger_1.logger.error('jsPDF generation error:', error);
        throw error;
    }
}
