...
class McpClient {
    ...
    async searchAccountingInfo(query) {
        try {
    ...
    // 国税庁のガイドラインを検索
    async searchTaxGuidelines(keyword) {
        try {
            ...
            logger_1.logger.error('Tax guidelines search error:', error);
            return '';
        }
    }
    // 業界別の勘定科目慣習を取得
    async getIndustryPractices(industry, transactionType) {
        try {
            ...
            logger_1.logger.error('Industry practices search error:', error);
            return '{}';
        }
    }
    // 類似取引の勘定科目を検索
    async searchSimilarTransactions(description, amount) {
        try {
            // MongoDBでテキスト検索を実装
            const { vercelDb } = await Promise.resolve().then(() => __importStar(require('@/lib/mongodb-client')));
            // テキスト検索またはパターンマッチング
            ...
            const documentsWithCategories = await Promise.all(documents.map(async (doc) => {
                if (doc.category) {
            ...
            logger_1.logger.error('Similar transactions search error:', error);
            return [];
        }
    }
}
