...
class DocumentGenerator {
    async generateFromNaturalLanguage(input, companyId) {
        // 自然言語から文書タイプを判定
    ...
    async extractPartner(input, companyId) {
        // パートナー名を抽出する正規表現パターン
        const patterns = [
        ...
        for (const pattern of patterns) {
            ...
            if (matches) {
                // データベースから取引先を検索
                const { data: partners } = await supabase_1.supabase
                    .from('partners')
                    .select('*')
    ...
    async extractItems(input, companyId) {
        const items = [];
        // 金額パターン（税込み・税抜きの判定含む）
    ...
    async generateDocumentNumber(documentType, companyId) {
        const prefix = {
            estimate: 'EST',
            invoice: 'INV',
            delivery_note: 'DLV',
    ...
    extractProjectName(input, documentType) {
        // 件名を抽出
    ...
    calculateDueDate(issueDate, documentType) {
