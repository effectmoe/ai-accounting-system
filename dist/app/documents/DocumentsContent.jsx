...
exports.default = DocumentsContent;
const react_1 = require("react");
const navigation_1 = require("next/navigation");
const link_1 = __importDefault(require("next/link"));
const lucide_react_1 = require("lucide-react");
const react_hot_toast_1 = require("react-hot-toast");
const constants_1 = require("@/components/common/constants");
function DocumentsContent() {
    console.log('🔴🔴🔴 DocumentsContent コンポーネントがマウントされました！');
    const router = (0, navigation_1.useRouter)();
    const searchParams = (0, navigation_1.useSearchParams)();
    const tabFromUrl = searchParams.get('tab');
    const [activeTab, setActiveTab] = (0, react_1.useState)(tabFromUrl || 'ocr');
    const [debugInfo, setDebugInfo] = (0, react_1.useState)(['DocumentsContent mounted at ' + new Date().toISOString()]);
    const [documents, setDocuments] = (0, react_1.useState)([]);
    const [ocrResults, setOcrResults] = (0, react_1.useState)([]);
    ...
    const [totalOcrPages, setTotalOcrPages] = (0, react_1.useState)(1);
    const [viewMode, setViewMode] = (0, react_1.useState)('card');
    const [ocrSortBy, setOcrSortBy] = (0, react_1.useState)('date');
    const [ocrSortOrder, setOcrSortOrder] = (0, react_1.useState)('desc');
    const [ocrFilters, setOcrFilters] = (0, react_1.useState)({
        vendor: '',
        dateFrom: '',
        dateTo: '',
        minAmount: '',
    ...
    // OCR結果を取得
    const fetchOcrResults = (0, react_1.useCallback)(async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                react_hot_toast_1.toast.error('OCR結果の取得に失敗しました');
            }
        }
        catch (error) {
            console.error('🚨 OCR結果取得エラー:', error);
            console.error('🚨 エラースタック:', error.stack);
            react_hot_toast_1.toast.error('OCR結果の取得中にエラーが発生しました');
        }
    }, [currentPage, documentsPerPage, ocrSortBy, ocrSortOrder, ocrFilters]);
    // 文書一覧を取得
    const fetchDocuments = (0, react_1.useCallback)(async () => {
        try {
            ...
            if (data.success) {
                ...
                react_hot_toast_1.toast.error('文書の取得に失敗しました');
            }
        }
        catch (error) {
            console.error('Error fetching documents:', error);
            react_hot_toast_1.toast.error('文書の取得中にエラーが発生しました');
        }
    }, [filters, currentPage]);
    // 文書化
    const handleCreateDocument = async (ocrResult, documentType = 'receipt') => {
        try {
            // API経由で文書作成（シンプル版）
            ...
            if (response.ok) {
                react_hot_toast_1.toast.success(`${result.message || '領収書を作成しました'}`);
                // ローカルステートを即座に更新
                setOcrResults(prev => prev.map(ocr => ocr.id === ocrResult.id
                ...
                react_hot_toast_1.toast.error(result.error || '文書の作成に失敗しました');
            }
        }
        catch (error) {
            console.error('Document creation error:', error);
            react_hot_toast_1.toast.error('文書の作成中にエラーが発生しました');
        }
    };
    // ステータス更新
    const handleStatusUpdate = async (documentId, newStatus) => {
        try {
            ...
            if (response.ok) {
                ...
                react_hot_toast_1.toast.error(error.error || 'ステータスの更新に失敗しました');
            }
        }
        catch (error) {
            console.error('Status update error:', error);
            react_hot_toast_1.toast.error('ステータスの更新中にエラーが発生しました');
        }
    };
    // OCR結果を削除
    const handleDeleteOcrResult = async (ocrResult) => {
        if (!confirm('このOCR結果を削除しますか？'))
            return;
        try {
            ...
            react_hot_toast_1.toast.success('OCR結果を削除しました');
            await fetchOcrResults();
        }
        catch (error) {
            console.error('Delete error:', error);
            react_hot_toast_1.toast.error(error instanceof Error ? error.message : '削除に失敗しました');
        }
    };
    // 選択モードの切り替え
    ...
    const handleDeleteSelected = async () => {
        ...
        try {
            ...
            react_hot_toast_1.toast.success(`${selectedItems.size}件のデータを削除しました`);
            setSelectedItems(new Set());
            ...
            console.error('Delete error:', error);
            react_hot_toast_1.toast.error('削除中にエラーが発生しました');
        }
    };
    // URLパラメータの変更を監視
    (0, react_1.useEffect)(() => {
    ...
    const filteredAndSortedOcrResults = (0, react_1.useCallback)(() => {
        return ocrResults;
    }, [ocrResults]);
    return (<div className="flex h-screen bg-gray-100">
      <div className="flex-1 flex flex-col">
          ...
          <div className="px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center py-4">
              ...
              <div className="flex items-center space-x-4">
                <link_1.default href="/documents/new" className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700">
          ...
          <div className="px-4 sm:px-6 lg:px-8 py-8">
            ...
            if (id) {
              ...
              {ocrResults.length > 0 && (<>
                  ...
                  <div className="flex gap-2 mt-2">
                    {Array.from({ length: Math.min(3, totalPages) }, (_, i) => i + 1).map(page => (<button key={page} onClick={() => setCurrentPage(page)} className={`px-3 py-1 text-sm rounded ${currentPage === page
                    ? 'bg-blue-500 text-white'
                    : 'bg-gray-200 hover:bg-gray-300'}`} disabled={loading}>
                        ページ{page}
                      </button>))}
                  </div>
                  <details className="mt-2">
                    ...
                    <div className="mt-2 text-xs max-h-40 overflow-auto">
                      {filteredAndSortedOcrResults().slice(0, 3).map((result, index) => (<div key={index} className="mb-2 p-2 bg-white rounded">
            ...
            <div className="mb-6">
              <nav className="flex space-x-4">
                <button onClick={() => handleTabChange('ocr')} className={`px-4 py-2 text-sm font-medium rounded-md ${activeTab === 'ocr'
            ? 'bg-blue-100 text-blue-700'
            : 'text-gray-500 hover:text-gray-700'}`}>
                  OCR結果
                </button>
                <button onClick={() => handleTabChange('documents')} className={`px-4 py-2 text-sm font-medium rounded-md ${activeTab === 'documents'
            ? 'bg-blue-100 text-blue-700'
            : 'text-gray-500 hover:text-gray-700'}`}>
                  作成済み文書
                </button>
            ...
            {/* フィルター（作成済み文書タブ） */}
            {activeTab === 'documents' && (<div className="bg-white rounded-lg shadow mb-6 p-4">
                {/* 選択モードボタン */}
                <div className="flex justify-between items-center mb-4">
                  <div className="flex gap-2">
                    {!isSelectionMode ? (<button onClick={toggleSelectionMode} className="px-3 py-1 border border-gray-300 rounded-md text-sm hover:bg-gray-50">
                        <lucide_react_1.CheckSquare className="h-4 w-4 inline mr-1"/>
                        選択モード
                      </button>) : (<>
                        <button onClick={toggleSelectAll} className="px-3 py-1 border border-gray-300 rounded-md text-sm hover:bg-gray-50">
                          {selectedItems.size === documents.length ? '全解除' : '全選択'}
                        </button>
                        <button onClick={handleDeleteSelected} disabled={selectedItems.size === 0} className="px-3 py-1 bg-red-500 text-white rounded-md text-sm hover:bg-red-600 disabled:bg-gray-300 disabled:cursor-not-allowed">
                          <lucide_react_1.Trash2 className="h-4 w-4 inline mr-1"/>
                          選択した{selectedItems.size}件を削除
                        </button>
                        <button onClick={toggleSelectionMode} className="px-3 py-1 border border-gray-300 rounded-md text-sm hover:bg-gray-50">
                          <lucide_react_1.X className="h-4 w-4 inline mr-1"/>
                          キャンセル
                        </button>
                      </>)}
                  </div>
                  {totalPages > 1 && (<span className="text-sm text-gray-500">
                ...
                <div className="flex items-center justify-between flex-wrap gap-4">
                  <div className="flex items-center gap-4 flex-wrap">
                    ...
                    <select value={filters.documentType} onChange={(e) => setFilters({ ...filters, documentType: e.target.value })} className="px-3 py-1 border border-gray-300 rounded-md text-sm">
                    ...
                    <select value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })} className="px-3 py-1 border border-gray-300 rounded-md text-sm">
                    ...
                    <input type="date" value={filters.dateFrom} onChange={(e) => setFilters({ ...filters, dateFrom: e.target.value })} className="px-3 py-1 border border-gray-300 rounded-md text-sm" placeholder="開始日"/>

                    <input type="date" value={filters.dateTo} onChange={(e) => setFilters({ ...filters, dateTo: e.target.value })} className="px-3 py-1 border border-gray-300 rounded-md text-sm" placeholder="終了日"/>

                    <button onClick={() => setFilters({ documentType: '', status: '', dateFrom: '', dateTo: '' })} className="px-3 py-1 text-sm text-gray-600 hover:text-gray-900">
                      クリア
                    </button>
                  </div>
                  ...
                  {/* ビューモード切り替え */}
                  <div className="flex items-center gap-2">
                    <button onClick={() => setViewMode('card')} className={`p-2 rounded-md transition-colors ${viewMode === 'card'
                ? 'bg-blue-100 text-blue-700'
                : 'text-gray-500 hover:text-gray-700'}`} title="カード表示">
                      <lucide_react_1.Grid3X3 className="h-4 w-4"/>
                    </button>
                    <button onClick={() => setViewMode('table')} className={`p-2 rounded-md transition-colors ${viewMode === 'table'
                ? 'bg-blue-100 text-blue-700'
                : 'text-gray-500 hover:text-gray-700'}`} title="テーブル表示">
            ...
            {/* フィルター（OCRタブ） */}
            {activeTab === 'ocr' && (<div className="bg-white rounded-lg shadow mb-6 p-4">
                {/* 選択モードボタン */}
                <div className="flex justify-between items-center mb-4">
                  <div className="flex gap-2">
                    {!isSelectionMode ? (<button onClick={toggleSelectionMode} className="px-3 py-1 border border-gray-300 rounded-md text-sm hover:bg-gray-50">
                  ...
                  {totalOcrPages > 1 && (<span className="text-sm text-gray-500">
                ...
                <div className="flex items-center justify-between flex-wrap gap-4">
                  <div className="flex items-center gap-4 flex-wrap">
                    ...
                    <select value={ocrSortBy} onChange={(e) => {
                setOcrSortBy(e.target.value);
                setCurrentPage(1); // ソート変更時はページを1に戻す
                    ...
                    <select value={ocrSortOrder} onChange={(e) => {
                setOcrSortOrder(e.target.value);
                setCurrentPage(1); // ソート変更時はページを1に戻す
            }} className="px-3 py-1 border border-gray-300 rounded-md text-sm">
                    ...
                    <select value={ocrFilters.documentType || ''} onChange={(e) => {
                ...
                setCurrentPage(1); // フィルター変更時はページを1に戻す
                    ...
                    <input type="text" value={ocrFilters.vendor} onChange={(e) => {
                setOcrFilters({ ...ocrFilters, vendor: e.target.value });
                setCurrentPage(1); // フィルター変更時はページを1に戻す
            }} className="px-3 py-1 border border-gray-300 rounded-md text-sm" placeholder="取引先で検索"/>

                    <input type="date" value={ocrFilters.dateFrom} onChange={(e) => {
                setOcrFilters({ ...ocrFilters, dateFrom: e.target.value });
                setCurrentPage(1); // フィルター変更時はページを1に戻す
            }} className="px-3 py-1 border border-gray-300 rounded-md text-sm" placeholder="開始日"/>

                    <input type="date" value={ocrFilters.dateTo} onChange={(e) => {
                setOcrFilters({ ...ocrFilters, dateTo: e.target.value });
                setCurrentPage(1); // フィルター変更時はページを1に戻す
            }} className="px-3 py-1 border border-gray-300 rounded-md text-sm" placeholder="終了日"/>
        ...
        (() => {
            ...
            return displayResults.length === 0 ? (<div className="p-8 text-center">
                    ...
                    </div>) : viewMode === 'card' ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-4 md:p-6">
                      {displayResults.map((result, index) => (<div key={result.id} className="bg-white border border-gray-200 rounded-lg hover:shadow-lg transition-all duration-200 min-h-[200px] relative">
                        {/* 選択チェックボックス */}
                        {isSelectionMode && (<div className="absolute top-2 left-2 z-10">
                            ...
                            toggleItemSelection(result.id);
                              ...
                              {selectedItems.has(result.id) ? (<lucide_react_1.CheckSquare className="h-5 w-5 text-blue-600"/>) : (<lucide_react_1.Square className="h-5 w-5 text-gray-400"/>)}
                            </button>
                          </div>)}
                        <div className="p-4">
                          {/* デバッグ情報 */}
                          ...
                          <div className="mb-3">
                            ...
                            {result.store_phone && (<div className="text-xs text-gray-500 mt-0.5">TEL: {result.store_phone}</div>)}
                          </div>
                        ...
                        : Math.max(0, (result.total_amount || 0) - (result.tax_amount || 0))).toLocaleString()}
                                </span>
                              </div>
                              <div className="flex justify-between text-xs">
                        ...
                        : '-'}
                          ...
                          <div className="flex justify-between items-center mb-3">
                            <div className="flex items-center gap-2 text-xs">
                              ...
                              {result.document_type && (<span className="inline-block px-2 py-0.5 bg-blue-100 text-blue-700 rounded">
                                  {constants_1.documentTypeLabels[result.document_type] || result.document_type}
                                </span>)}
                            </div>
                        ...
                        minute: '2-digit'
                          ...
                          {/* 勘定科目情報 */}
                          {result.category && (<div className="mb-3">
                              <div className="flex items-center gap-2">
                                ...
                                {result.aiPrediction?.confidence && (<span className="text-xs text-gray-500">
                          ...
                          {/* 備考 */}
                          {result.notes && (<div className="mb-3">
                          ...
                          <details className="mb-3">
                            ...
                            <div className="mt-2 space-y-1 text-xs text-gray-600">
                              ...
                              {result.extracted_text && (<div className="mt-2">
                          ...
                          <div className="pt-3 border-t border-gray-200">
                            {result.linked_document_id ? (<div className="space-y-2">
                                <div className="flex items-center justify-between">
                                  <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                    <lucide_react_1.CheckCircle className="h-3 w-3 mr-0.5"/>
                                    文書化済
                                  </span>
                                  {result.linked_document_id && (<link_1.default href={`/documents/${result.linked_document_id}`} className="px-3 py-1.5 border border-gray-300 rounded-md text-xs font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors">
                                      詳細を見る
                                    </link_1.default>)}
                                </div>
                                <button onClick={() => handleDeleteOcrResult(result)} className="block w-full px-3 py-2 border border-red-300 rounded-md text-sm font-medium text-red-700 bg-white hover:bg-red-50 transition-colors">
                                  <lucide_react_1.Trash2 className="inline-block h-4 w-4 mr-1"/>
                                  削除
                                </button>
                                ...
                                <div className="flex items-center justify-between gap-2">
                                  ...
                                  <select id={`doc-type-${result.id}`} className="flex-1 px-2 py-1 text-xs border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 bg-white" defaultValue="receipt">
                                    <option value="receipt">領収書</option>
                                    <option value="invoice">請求書</option>
                                    <option value="estimate">見積書</option>
                                    <option value="delivery_note">納品書</option>
                                ...
                                <button onClick={() => {
                            const select = document.getElementById(`doc-type-${result.id}`);
            ...
            <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                          <tr>
                            {isSelectionMode && (<th className="px-6 py-3 text-center">
                                <button onClick={toggleSelectAll} className="p-1 hover:bg-gray-200 rounded">
                                  {selectedItems.size === displayResults.length ? (<lucide_react_1.CheckSquare className="h-4 w-4 text-blue-600"/>) : (<lucide_react_1.Square className="h-4 w-4 text-gray-400"/>)}
                                </button>
                              </th>)}
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {displayResults.map((result) => (<tr key={result.id} className="hover:bg-gray-50">
                              {isSelectionMode && (<td className="px-6 py-4 text-center">
                                  <button onClick={() => toggleItemSelection(result.id)} className="p-1 hover:bg-gray-100 rounded">
                                    {selectedItems.has(result.id) ? (<lucide_react_1.CheckSquare className="h-4 w-4 text-blue-600"/>) : (<lucide_react_1.Square className="h-4 w-4 text-gray-400"/>)}
                                  </button>
                                </td>)}
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-center">
                                {result.linked_document_id ? (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                    文書化済
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                {result.linked_document_id ? (<link_1.default href={`/documents/${result.linked_document_id}`} className="text-blue-600 hover:text-blue-900">
                                    詳細を見る
                                  </link_1.default>) : (<button onClick={() => {
                            handleCreateDocument(result, 'receipt');
                        }} className="text-blue-600 hover:text-blue-900" disabled={isSelectionMode}>
                                    文書化
                                  </button>)}
                                {!isSelectionMode && (<button onClick={() => handleDeleteOcrResult(result)} className="text-red-600 hover:text-red-900 ml-3">
                                    削除
                                  </button>)}
                              </td>
                            </tr>))}
        ...
        // 作成済み文書
        documents.length === 0 ? (<div className="p-8 text-center">
                    <lucide_react_1.FileText className="mx-auto h-12 w-12 text-gray-400"/>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-4 md:p-6">
                    {documents.map((doc) => (<div key={doc.id} className="bg-white border border-gray-200 rounded-lg hover:shadow-lg transition-all duration-200 relative">
                        {/* 選択チェックボックス */}
                        {isSelectionMode && (<div className="absolute top-2 left-2 z-10">
                            <button onClick={(e) => {
                        e.stopPropagation();
                        toggleItemSelection(doc.id);
                              ...
                              {selectedItems.has(doc.id) ? (<lucide_react_1.CheckSquare className="h-5 w-5 text-blue-600"/>) : (<lucide_react_1.Square className="h-5 w-5 text-gray-400"/>)}
                            </button>
                          </div>)}
                        <div className="p-4">
                          {/* ヘッダー */}
                          <div className="flex justify-between items-start mb-3">
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-gray-500 mt-0.5">
                                {constants_1.documentTypeLabels[(doc.documentType || doc.document_type || doc.type)] || '領収書'}
                              </p>
                            </div>
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${constants_1.statusColors[doc.status] || 'bg-gray-100 text-gray-800'}`}>
                              {constants_1.statusLabels[doc.status] || doc.status || '下書き'}
                            </span>
                          </div>
                          ...
                          <div className="mb-3">
                            ...
                            {doc.company_name && (<div className="text-xs text-gray-600">{doc.company_name}</div>)}
                            {doc.store_phone && (<div className="text-xs text-gray-500">TEL: {doc.store_phone}</div>)}
                          </div>
                    ...
                    Math.max(0, (doc.totalAmount || doc.total_amount || 0) - (doc.tax_amount || 0))).toLocaleString()}
                                </span>
                              </div>
                              <div className="flex justify-between text-xs">
                                <span className="text-gray-600">税額</span>
                    ...
                    : '-'}
                          ...
                          {/* 勘定科目情報 */}
                          {doc.category && (<div className="mb-3">
                              <div className="flex items-center gap-2">
                                <span className="text-xs font-medium text-gray-700">勘定科目:</span>
                                <span className="inline-block px-2 py-0.5 bg-green-100 text-green-800 text-xs rounded">
                                  {doc.category}
                                </span>
                                {doc.aiPrediction?.confidence && (<span className="text-xs text-gray-500">
                    ...
                    minute: '2-digit'
                          ...
                          {/* アクション */}
                          <div className="pt-3 border-t border-gray-200">
                            <link_1.default href={`/documents/${doc.id}`} className="block w-full text-center px-3 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors">
                              詳細を見る
                            </link_1.default>
                          </div>
                        </div>
        ...
        <div className="overflow-x-auto">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {documents.map((doc) => (<tr key={doc.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                              {doc.category ? (<span className="inline-block px-2 py-0.5 bg-green-100 text-green-800 text-xs rounded">
                                  {doc.category}
                                </span>) : (<span className="text-gray-400">未設定</span>)}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    ...
                    new Date(doc.issueDate || doc.issue_date || doc.receipt_date).toLocaleDateString('ja-JP') : '-'}
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-center">
                              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${constants_1.statusColors[doc.status] || 'bg-gray-100 text-gray-800'}`}>
                                {constants_1.statusLabels[doc.status] || doc.status || '下書き'}
                              </span>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                              <link_1.default href={`/documents/${doc.id}`} className="text-blue-600 hover:text-blue-900">
                                詳細を見る
                              </link_1.default>
                            </td>
                          </tr>))}
            ...
            {/* ページネーション */}
            {(activeTab === 'ocr' ? totalOcrPages : totalPages) > 1 && (<div className="mt-6 flex justify-center">
                <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                  ...
                  {[...Array(activeTab === 'ocr' ? totalOcrPages : totalPages)].map((_, i) => (<button key={i + 1} onClick={() => setCurrentPage(i + 1)} className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${currentPage === i + 1
                    ? 'z-10 bg-blue-50 border-blue-500 text-blue-600'
                    : 'bg-white border-gray-300 text-gray-500 hover:bg-gray-50'}`}>
                      {i + 1}
                    </button>))}
                  
                  <button onClick={() => setCurrentPage(Math.min(activeTab === 'ocr' ? totalOcrPages : totalPages, currentPage + 1))} disabled={currentPage === (activeTab === 'ocr' ? totalOcrPages : totalPages)} className="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50">
                    次へ
                  </button>
                </nav>
              </div>)}
