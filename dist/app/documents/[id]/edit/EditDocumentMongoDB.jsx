...
function EditDocumentMongoDB() {
    ...
    const fetchDocument = async () => {
        try {
            logger_1.logger.debug('Fetching document with ID:', documentId);
            // 単一ドキュメントを取得するためのAPIエンドポイントを使用
            const response = await fetch(`/api/documents/${documentId}`);
            const data = await response.json();
            logger_1.logger.debug('Single document response:', data);
            if (data.success) {
                logger_1.logger.debug('Found document:', data.document);
