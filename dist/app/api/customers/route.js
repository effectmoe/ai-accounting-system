...
exports.POST = (0, unified_error_handler_1.withErrorHandler)(async (request) => {
    ...
    // アクティビティログを記録
    try {
        await activity_log_service_1.ActivityLogService.logCustomerCreated(result.insertedId.toString(), newCustomer.companyName);
        logger_1.logger.info('Activity log recorded for customer creation');
    }
    catch (logError) {
        logger_1.logger.error('Failed to log activity for customer creation:', logError);
    }
    return server_1.NextResponse.json({
        success: true,
