...
async function GET(request, { params }) {
    try {
        logger_1.logger.debug('[GET /api/invoices/[id]] Request for ID:', params.id);
        const invoiceService = new invoice_service_1.InvoiceService();
        const invoice = await invoiceService.getInvoice(params.id);
        if (!invoice) {
            logger_1.logger.debug('[GET /api/invoices/[id]] Invoice not found for ID:', params.id);
            return server_1.NextResponse.json({ error: 'Invoice not found' }, { status: 404 });
        }
        logger_1.logger.debug('[GET /api/invoices/[id]] Invoice found:', {
            id: invoice._id,
        ...
        logger_1.logger.debug('[GET /api/invoices/[id]] Formatted invoice status:', formattedInvoice.status);
        logger_1.logger.debug('[GET /api/invoices/[id]] Formatted invoice convertedToDeliveryNoteId:', formattedInvoice.convertedToDeliveryNoteId);
...
async function PUT(request, { params }) {
    try {
        logger_1.logger.debug('[PUT /api/invoices/[id]] Request for ID:', params.id);
        const body = await request.json();
        logger_1.logger.debug('[PUT /api/invoices/[id]] Request body:', body);
        const invoiceService = new invoice_service_1.InvoiceService();
        // ステータス更新の場合
        if (body.status && Object.keys(body).length === 1) {
            logger_1.logger.debug('Updating invoice status:', params.id, body.status);
            const invoice = await invoiceService.updateInvoiceStatus(params.id, body.status);
            if (!invoice) {
                logger_1.logger.debug('Invoice not found for status update:', params.id);
                return server_1.NextResponse.json({ error: 'Invoice not found' }, { status: 404 });
            }
            logger_1.logger.debug('Invoice status updated successfully');
        ...
        // 通常の更新
        logger_1.logger.debug('Updating invoice with data:', body);
        // 請求書データの整形
        const updateData = {};
        if (body.customerId)
        ...
        logger_1.logger.debug('[PUT /api/invoices/[id]] Calling updateInvoice with ID:', params.id);
        logger_1.logger.debug('[PUT /api/invoices/[id]] Update data:', JSON.stringify(updateData, null, 2));
        const invoice = await invoiceService.updateInvoice(params.id, updateData);
        if (!invoice) {
            logger_1.logger.error('[PUT /api/invoices/[id]] Invoice not found. ID:', params.id);
            logger_1.logger.error('[PUT /api/invoices/[id]] Attempted update data:', updateData);
            return server_1.NextResponse.json({ error: 'Invoice not found' }, { status: 404 });
        }
        logger_1.logger.debug('[PUT /api/invoices/[id]] Invoice updated successfully');
        return server_1.NextResponse.json(invoice);
    }
    catch (error) {
