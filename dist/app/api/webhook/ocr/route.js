...
async function POST(request) {
    try {
        logger_1.logger.debug('OCR Webhook received - start');
        const data = await request.json();
        logger_1.logger.debug('OCR Webhook data:', JSON.stringify(data, null, 2));
        logger_1.logger.debug('Document type from GAS:', data.documentType || 'not specified');
        // MongoDBのみを使用（Supabaseは削除）
        {
            // MongoDB処理
            logger_1.logger.debug('MongoDB処理を開始');
            // MongoDB接続確認
            let mongoConnected = false;
            try {
                logger_1.logger.debug('Checking MongoDB connection...');
                logger_1.logger.debug('MONGODB_URI exists:', !!process.env.MONGODB_URI);
                logger_1.logger.debug('NODE_ENV:', process.env.NODE_ENV);
                mongoConnected = await (0, mongodb_client_1.checkConnection)();
            }
            catch (connectionError) {
                logger_1.logger.error('MongoDB接続チェックエラー:', connectionError);
                logger_1.logger.error('Error stack:', connectionError instanceof Error ? connectionError.stack : 'No stack trace');
            ...
            if (!mongoConnected) {
                logger_1.logger.error('MongoDB接続に失敗しました');
                return server_1.NextResponse.json({
                    success: false,
                    error: 'データベース接続エラー: MongoDBに接続できません'
                }, { status: 500 });
            }
            logger_1.logger.debug('MongoDB接続確認完了');
            ...
            logger_1.logger.debug('Detected document type:', detectedDocumentType);
            // OCR結果をMongoDBに保存
            const ocrResult = {
            ...
            let savedOcrResult;
            try {
                savedOcrResult = await mongodb_client_1.vercelDb.create('ocr_results', ocrResult);
                logger_1.logger.debug('OCR結果をMongoDBに保存しました:', savedOcrResult._id.toString());
            }
            catch (mongoError) {
                logger_1.logger.error('MongoDB OCR結果保存エラー:', mongoError);
                return server_1.NextResponse.json({
                    success: false,
            ...
            const documentData = {
                ...
                confidence: 0.8,
                ocrStatus: 'completed',
                ocrResultId: savedOcrResult._id ? new mongodb_1.ObjectId(savedOcrResult._id) : null,
                gridfsFileId: null,
                status: 'pending',
                createdAt: new Date(),
                updatedAt: new Date()
            ...
            try {
                const savedDocument = await mongodb_client_1.vercelDb.create('documents', documentData);
                logger_1.logger.debug('Document saved to MongoDB:', savedDocument._id.toString());
            }
            catch (mongoError) {
                logger_1.logger.error('MongoDB documents保存エラー:', mongoError);
                return server_1.NextResponse.json({
                    success: false,
        ...
        logger_1.logger.error('Webhook処理エラー:', error);
        logger_1.logger.error('エラーの詳細:', {
