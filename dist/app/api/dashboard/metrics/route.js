...
async function GET(request) {
    ...
    try {
        ...
        logger_1.logger.info(`Total revenue calculated: ¥${totalRevenue.toLocaleString()}`);
        // 2. 処理済みドキュメント数
        ...
        logger_1.logger.info(`Processed documents count: ${processedDocuments}`);
        // 3. 保留中のドキュメント数
        ...
        logger_1.logger.info(`Pending documents count: ${pendingDocuments}`);
        // 4. アクティブな顧客数/仕入先数
        ...
        logger_1.logger.info(`Active customers count: ${activeCustomers}`);
        // 5. 最近のアクティビティ
        ...
        logger_1.logger.info(`Recent activities count: ${recentActivities.length}`);
        const metrics = {
            totalRevenue,
            processedDocuments,
            pendingDocuments,
        ...
        logger_1.logger.info('Dashboard metrics successfully compiled', {
            totalRevenue: metrics.totalRevenue,
            processedDocuments: metrics.processedDocuments,
            pendingDocuments: metrics.pendingDocuments,
            activeCustomers: metrics.activeCustomers,
        ...
        return server_1.NextResponse.json({ error: 'Failed to fetch dashboard metrics' }, { status: 500 });
    }
}
