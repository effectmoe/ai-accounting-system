...
async function GET(request) {
    try {
        // 詳細なデバッグログ
        logger_1.logger.debug('===== Journals API GET Request Start =====');
        ...
        logger_1.logger.debug('Query parameters:', { companyId, dateFrom, dateTo, limit, skip });
        // フィルター条件を構築
        const filter = {};
        ...
        logger_1.logger.debug('Filter object:', filter);
        // 仕訳を取得
        logger_1.logger.debug('Attempting to fetch journals from database...');
        const journals = await mongodb_client_1.db.find('journals', filter, {
            limit,
            skip,
            sort: { entryDate: -1, createdAt: -1 }
        });
        logger_1.logger.debug('Journals fetched successfully:', {
            count: journals.length,
            hasData: journals.length > 0,
            firstJournal: journals.length > 0 ? journals[0] : null
        });
        // 総数を取得
        logger_1.logger.debug('Fetching total count...');
        const totalCount = await mongodb_client_1.db.count('journals', filter);
        logger_1.logger.debug('Total count:', totalCount);
        const response = {
            success: true,
            journals,
            totalCount,
        ...
        logger_1.logger.debug('===== Journals API GET Request End =====');
        return server_1.NextResponse.json(response);
    }
    catch (error) {
        logger_1.logger.error('===== Journal API Error =====');
...
exports.maxDuration = 30; // Vercel function timeout in seconds
