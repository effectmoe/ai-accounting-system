...
async function POST(request) {
    try {
        ...
        logger_1.logger.info('MongoDB configuration check:', {
        ...
        if (!useAzureMongoDB && !process.env.MONGODB_URI) {
            logger_1.logger.warn('MongoDB is not enabled and no URI found. Returning error.');
        ...
        if (process.env.MONGODB_URI && !useAzureMongoDB) {
            logger_1.logger.info('MongoDB URI exists but USE_AZURE_MONGODB is not true. Proceeding anyway.');
        }
        logger_1.logger.info('Creating journal entry with request body');
        ...
        // MongoDBに保存
        logger_1.logger.info('Saving journal entry to MongoDB:', { journalNumber });
        const savedJournal = await mongodb_client_1.db.create('journals', journalEntry);
        logger_1.logger.info('Journal entry saved successfully:', { journalId: savedJournal._id });
        // 関連ドキュメントを更新（存在する場合）
        ...
        if (documentId) {
            ...
            logger_1.logger.debug('Updated source document with journal number:', {
                documentId,
                journalNumber,
