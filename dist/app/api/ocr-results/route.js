...
async function GET(request) {
    ...
    try {
        ...
        const ocrResultsFromOcrResults = await mongodb_client_1.db.find('ocr_results', filter, {
            limit: Math.ceil(limit * 2),
            skip: 0,
            sort: { createdAt: -1, _id: -1 }
        });
        const ocrResultsFromDocuments = await mongodb_client_1.db.find('documents', filter, {
            limit: Math.ceil(limit * 2),
            skip: 0,
            sort: { createdAt: -1, _id: -1 }
        });
        ...
        // 重複削除（_idベース）
        const uniqueResults = allResults.filter((item, index, self) => index === self.findIndex(t => t._id.toString() === item._id.toString()));
        // 日付でソート（最新順 = 降順）
        ...
        if (ocrResults.length > 0) {
            ...
            if (page === 2) {
                ...
                const previousPageLast = await mongodb_client_1.db.find('ocr_results', filter, {
                    limit: 3,
                    skip: limit - 3,
                    sort: sortOptions
                });
        ...
        for (const targetId of targetIds) {
            try {
                const targetDoc = await mongodb_client_1.db.findOne('ocr_results', { _id: new mongodb_1.ObjectId(targetId) });
                if (targetDoc) {
                    const checks = {
        ...
        // 総数を取得（両方のコレクションから）
        const totalOcrResults = await mongodb_client_1.db.count('ocr_results', filter);
        const totalDocuments = await mongodb_client_1.db.count('documents', filter);
        ...
        if (page === 1 && total > limit) {
            ...
            // 最初の5件
            const firstFive = await mongodb_client_1.db.find('ocr_results', filter, {
                limit: 5,
                skip: 0,
                sort: sortOptions
            });
            ...
            // 最後の5件
            const lastFive = await mongodb_client_1.db.find('ocr_results', filter, {
                limit: 5,
        ...
        if (page === 1 && total > limit) {
            ...
            const allRecent = await mongodb_client_1.db.find('ocr_results', filter, {
                limit: 5,
                skip: 0,
                sort: { createdAt: -1 }
            });
