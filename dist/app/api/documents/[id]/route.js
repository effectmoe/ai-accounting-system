...
async function GET(request, { params }) {
    try {
        ...
        if (id && id.startsWith('J')) {
            logger_1.logger.debug('Searching by displayNumber:', id);
            document = await mongodb_client_1.db.findOne('documents', { displayNumber: id });
            if (!document) {
                logger_1.logger.debug('Document not found by displayNumber, trying as regular ID');
            }
        }
        ...
        if (!document) {
            ...
            // ドキュメントを取得
            document = await mongodb_client_1.db.findById('documents', id);
        }
        if (!document) {
            return server_1.NextResponse.json({
                success: false,
        ...
        logger_1.logger.debug('Raw MongoDB document:', {
            _id: document._id,
