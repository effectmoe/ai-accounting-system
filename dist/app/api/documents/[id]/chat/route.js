...
async function POST(request, { params }) {
    try {
        ...
        if (!message || !documentData) {
            return server_1.NextResponse.json({ error: 'メッセージと文書データが必要です' }, { status: 400 });
        }
        // 文書の種類に応じた説明を生成
...
`;
        // デバッグログを追加
        logger_1.logger.info('Document chat request:', {
...
${documentContext}`;
        ...
        if (!openaiResponse.ok) {
            ...
            logger_1.logger.error('OpenAI API error:', errorData);
        ...
        logger_1.logger.error('Document chat error:', error);
        return server_1.NextResponse.json({ error: error instanceof Error ? error.message : '処理中にエラーが発生しました' }, { status: 500 });
    }
}
