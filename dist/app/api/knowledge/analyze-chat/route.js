...
async function callDeepSeekAPI(messages, temperature = 0.7, maxTokens = 1000) {
    ...
    logger_1.logger.debug('[DeepSeek] Calling API for knowledge chat');
    ...
    try {
        ...
        logger_1.logger.debug('[DeepSeek] Knowledge chat API response received');
        return data;
    }
    catch (error) {
        clearTimeout(timeoutId);
...
async function POST(request) {
    try {
        logger_1.logger.debug('[Knowledge Chat] API called at', new Date().toISOString());
        const body = await request.json();
        const { conversation, conversationHistory = [], sessionId, includeKnowledge = true, knowledgeFilters = {}, stream = false } = body;
        ...
        if (includeKnowledge) {
            logger_1.logger.debug('[Knowledge Chat] Searching for relevant knowledge...');
...
以下の会話履歴とユーザーの質問に基づいて、専門的で実用的な回答を提供してください。`;
        ...
        logger_1.logger.debug('[Knowledge Chat] Prepared', messages.length, 'messages for AI');
        ...
        try {
            ...
            logger_1.logger.debug('[Knowledge Chat] Conversation history would be saved');
        }
        catch (error) {
            logger_1.logger.error('[Knowledge Chat] Failed to save conversation history:', error);
        }
        const result = {
            success: true,
            response,
        ...
        logger_1.logger.debug('[Knowledge Chat] Response generated successfully');
        return server_1.NextResponse.json(result);
    }
    catch (error) {
        logger_1.logger.error('[Knowledge Chat] Error:', error);
...
async function GET(request) {
    try {
        ...
        logger_1.logger.error('[Knowledge Chat] Get conversation error:', error);
        return server_1.NextResponse.json({ error: 'Failed to get conversation history' }, { status: 500 });
    }
}
