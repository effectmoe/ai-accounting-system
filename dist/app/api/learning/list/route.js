...
async function GET(request) {
    try {
        ...
        // 学習データを取得
        const learnings = await mongodb_client_1.db.find('accountLearningData', query, {
            sort: { updatedAt: -1, 'metadata.useCount': -1 },
            skip,
            limit
        });
        // 総件数を取得
        const total = await mongodb_client_1.db.count('accountLearningData', query);
        // 各学習データに統計情報を追加
        ...
        // カテゴリ別の統計を集計
        const categoryStats = await mongodb_client_1.db.aggregate('accountLearningData', [
            { $match: { companyId } },
            {
                $group: {
                    _id: '$accountCategory',
        ...
        logger_1.logger.error('Learning list error:', error);
        return server_1.NextResponse.json({
            success: false,
