...
exports.default = MastraAdminPage;
const react_1 = require("react");
const github_section_1 = __importDefault(require("./github-section"));
const problem_solving_section_1 = __importDefault(require("./problem-solving-section"));
const logger_1 = require("@/lib/logger");
function MastraAdminPage() {
    ...
    const loadDashboardData = async () => {
        try {
            ...
            // エージェント状態（モックデータ）
            const mockAgents = status.agents?.map((agent) => ({
                name: agent,
            ...
            logger_1.logger.error('Dashboard data load error:', error);
            setIsLoading(false);
        }
    };
    ...
    return (<div className="container mx-auto px-4 py-8">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {agents.map((agent) => (<tr key={agent.name}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${agent.status === 'active' ? 'bg-green-100 text-green-800' :
                agent.status === 'idle' ? 'bg-gray-100 text-gray-800' :
                    'bg-red-100 text-red-800'}`}>
                      <span className={`w-2 h-2 mr-1 rounded-full ${agent.status === 'active' ? 'bg-green-400' :
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="space-y-4">
            {workflows.map((workflow) => (<div key={workflow.name} className="border-b pb-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <span className={`ml-2 font-medium ${workflow.successRate >= 98 ? 'text-green-600' :
                ...
                <div className="mt-2">
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div className={`h-2 rounded-full ${workflow.successRate >= 98 ? 'bg-green-600' :
                workflow.successRate >= 95 ? 'bg-yellow-600' :
                    'bg-red-600'}`} style={{ width: `${workflow.successRate}%` }}></div>
                  </div>
                </div>
              </div>))}
          </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="space-y-2">
            {recentLogs.map((log, index) => (<div key={index} className="flex items-start space-x-2 text-sm">
                <span className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${log.level === 'success' ? 'bg-green-400' :
      ...
      {/* 問題解決専門エージェントセクション */}
      <problem_solving_section_1.default />

      {/* GitHub連携セクション */}
      <github_section_1.default />
    </div>);
}
