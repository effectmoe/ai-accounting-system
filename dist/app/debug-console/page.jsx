...
exports.default = DebugConsolePage;
...
function DebugConsolePage() {
    ...
    (0, react_1.useEffect)(() => {
        ...
        const checkGlobals = () => {
            const globalChecks = [
                ...
                { name: '__NEXT_DATA__', exists: typeof window?.__NEXT_DATA__ !== 'undefined' },
            ];
    ...
    return (<div className="p-8">
      ...
      <div className="mb-4 space-x-2">
        <button onClick={handleTestClick} className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">
          Test Click
        </button>
        
        <button onClick={handleForceError} className="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600">
          Force Error
        </button>
      ...
      <div className="bg-black text-green-400 p-4 rounded font-mono text-sm overflow-auto max-h-96">
        ...
        {logs.length === 0 ? (<p>No logs yet...</p>) : (logs.map((log, index) => (<div key={index} className="mb-1">
              [{index}] {log}
            </div>)))}
      </div>
