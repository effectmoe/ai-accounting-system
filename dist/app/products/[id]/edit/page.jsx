...
function EditProductPage({ params }) {
    ...
    const fetchProduct = async () => {
        try {
            ...
            logger_1.logger.error('商品取得エラー:', err);
    ...
    const fetchCategories = async () => {
        try {
            ...
            logger_1.logger.error('カテゴリ一覧取得エラー:', err);
        }
    };
    // フォーム送信
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            logger_1.logger.error('商品更新エラー:', err);
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              ...
              <input type="text" value={formData.tags.join(', ')} onChange={handleTagsChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="タグをカンマ区切りで入力（例：新商品,人気,季節限定）"/>
            </div>
          ...
          <div className="flex justify-end space-x-4 mt-8">
            ...
            <button type="submit" disabled={saving} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50">
              {saving ? '更新中...' : '商品を更新'}
            </button>
          </div>
        </form>
