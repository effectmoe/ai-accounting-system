...
exports.default = SimpleDebugPage;
const react_1 = require("react");
function SimpleDebugPage() {
    const [count, setCount] = (0, react_1.useState)(0);
    const [apiData, setApiData] = (0, react_1.useState)(null);
    ...
    return (<div style={{ padding: '20px', fontFamily: 'monospace' }}>
      ...
      <div style={{ marginTop: '20px' }}>
        ...
        <pre style={{ backgroundColor: '#f0f0f0', padding: '10px', borderRadius: '4px' }}>
          ...
          React loaded: {react_1.useState ? 'Yes' : 'No'}
          Current time: {new Date().toLocaleTimeString()}
        </pre>
      </div>

      {error && (<div style={{ marginTop: '20px', color: 'red' }}>
          <h2>Error:</h2>
          <pre>{error}</pre>
        </div>)}

      {apiData && (<div style={{ marginTop: '20px' }}>
          <h2>API Response:</h2>
