...
function SuppliersPageContent() {
    ...
    const fetchSuppliers = (0, react_1.useCallback)(async () => {
        try {
            ...
            // キャッシュキーを生成
            const cacheKey = cache_1.SimpleCache.createKey('suppliers', params);
            // キャッシュをチェック
            const cachedData = cache_1.cache.get(cacheKey);
            if (cachedData) {
                logger_1.logger.debug('[Suppliers] Using cached data');
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (response.ok) {
                // キャッシュを無効化
                cache_1.cache.invalidate('suppliers');
    ...
    return (<div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex gap-4 items-center">
        <div className="flex-1 relative">
          ...
          <input_1.Input type="text" placeholder="会社名、コード、メールアドレスで検索..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" aria-label="仕入先検索" aria-describedby="search-help"/>
        </div>
...
function SuppliersPage() {
    return (<react_1.Suspense fallback={<div className="container mx-auto py-6 space-y-6"><div className="text-center py-8">読み込み中...</div></div>}>
