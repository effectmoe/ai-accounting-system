...
function EditSupplierPage() {
    ...
    (0, react_1.useEffect)(() => {
        const fetchSupplier = async () => {
            try {
                ...
                logger_1.logger.debug('Fetched supplier:', supplier);
                setFormData({
                ...
                logger_1.logger.error('Error loading supplier:', error);
    ...
    return (<div className="container mx-auto py-6 max-w-4xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <card_1.Card>
          <card_1.CardHeader>
            <card_1.CardTitle>振込先情報</card_1.CardTitle>
            <card_1.CardDescription>支払い時に使用する振込先口座情報</card_1.CardDescription>
          </card_1.CardHeader>
          <card_1.CardContent className="space-y-4">
            {formData.bankTransferInfo.map((bank, index) => (<div key={index} className="border rounded-lg p-4 space-y-4">
                <div className="flex justify-between items-center mb-2">
                  <h4 className="font-medium">振込先 {index + 1}</h4>
                  <button_1.Button type="button" variant="ghost" size="sm" onClick={() => removeBankInfo(index)} disabled={loading || saving}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label_1.Label htmlFor={`bankName-${index}`}>銀行名</label_1.Label>
                    <input_1.Input id={`bankName-${index}`} value={bank.bankName || ''} onChange={(e) => handleBankInfoChange(index, 'bankName', e.target.value)} placeholder="例: みずほ銀行" disabled={loading || saving}/>
                  </div>
                  <div>
                    <label_1.Label htmlFor={`branchName-${index}`}>支店名</label_1.Label>
                    <input_1.Input id={`branchName-${index}`} value={bank.branchName || ''} onChange={(e) => handleBankInfoChange(index, 'branchName', e.target.value)} placeholder="例: 東京支店" disabled={loading || saving}/>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label_1.Label htmlFor={`accountType-${index}`}>口座種別</label_1.Label>
                    <select_1.Select value={bank.accountType || ''} onValueChange={(value) => handleBankInfoChange(index, 'accountType', value)} disabled={loading || saving}>
                      <select_1.SelectTrigger>
                        <select_1.SelectValue placeholder="選択してください"/>
                      </select_1.SelectTrigger>
                      <select_1.SelectContent>
                        <select_1.SelectItem value="普通">普通</select_1.SelectItem>
                        <select_1.SelectItem value="当座">当座</select_1.SelectItem>
                        <select_1.SelectItem value="貯蓄">貯蓄</select_1.SelectItem>
                      </select_1.SelectContent>
                    </select_1.Select>
                  </div>
                  <div>
                    <label_1.Label htmlFor={`accountNumber-${index}`}>口座番号</label_1.Label>
                    <input_1.Input id={`accountNumber-${index}`} value={bank.accountNumber || ''} onChange={(e) => handleBankInfoChange(index, 'accountNumber', e.target.value)} placeholder="例: 1234567" disabled={loading || saving}/>
                  </div>
                </div>

                <div>
                  <label_1.Label htmlFor={`accountName-${index}`}>口座名義</label_1.Label>
                  <input_1.Input id={`accountName-${index}`} value={bank.accountName || ''} onChange={(e) => handleBankInfoChange(index, 'accountName', e.target.value)} placeholder="例: カブシキガイシャ エービーシー" disabled={loading || saving}/>
                </div>

                <div>
                  <label_1.Label htmlFor={`additionalInfo-${index}`}>その他情報</label_1.Label>
                  <textarea_1.Textarea id={`additionalInfo-${index}`} value={bank.additionalInfo || ''} onChange={(e) => handleBankInfoChange(index, 'additionalInfo', e.target.value)} rows={2} placeholder="特記事項があれば入力" disabled={loading || saving}/>
                </div>
              </div>))}

            <button_1.Button type="button" variant="outline" onClick={addBankInfo} disabled={loading || saving} className="w-full">
