...
exports.default = PurchaseInvoiceDetailPage;
...
function PurchaseInvoiceDetailPage({ params }) {
    ...
    const fetchInvoice = async () => {
        try {
            ...
            react_hot_toast_1.toast.error('請求書の取得に失敗しました');
    ...
    const handlePaymentUpdate = async () => {
        ...
        try {
            ...
            react_hot_toast_1.toast.success('支払情報を更新しました');
            setShowPaymentModal(false);
            fetchInvoice();
        }
        catch (error) {
            logger_1.logger.error('Error updating payment:', error);
            react_hot_toast_1.toast.error('支払情報の更新に失敗しました');
        }
        finally {
            setUpdating(false);
        }
    ...
    const handleDelete = async () => {
        if (!invoice || !confirm('この請求書を削除してもよろしいですか？'))
            return;
        try {
            const response = await fetch(`/api/purchase-invoices/${invoice._id}`, {
                method: 'DELETE',
            ...
            react_hot_toast_1.toast.success('請求書を削除しました');
            router.push('/purchase-invoices');
        }
        catch (error) {
            logger_1.logger.error('Error deleting invoice:', error);
            react_hot_toast_1.toast.error('請求書の削除に失敗しました');
        }
    };
    if (loading) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div className="flex items-center space-x-4">
          <link_1.default href="/purchase-invoices" className="text-gray-500 hover:text-gray-700">
          ...
          <div>
            ...
            <div className="mt-2 flex items-center space-x-4">
              ...
              {invoice.isGeneratedByAI && (<span className="text-blue-600 text-sm">
                  AI生成
                </span>)}
            </div>
          </div>
        ...
        <div className="flex items-center space-x-2">
          <link_1.default href={`/purchase-invoices/${invoice._id}/edit`} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 flex items-center space-x-2">
            <lucide_react_1.Edit className="w-4 h-4"/>
            <span>編集</span>
          </link_1.default>
          <button onClick={handleDelete} className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 flex items-center space-x-2">
      ...
      <div className="bg-white shadow rounded-lg p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
          ...
          {(invoice.supplier?.email || invoice.vendorEmail) && (<div>
              <p className="text-sm text-gray-500">メールアドレス</p>
              <p className="mt-1 text-sm font-medium text-gray-900">
                {invoice.supplier?.email || invoice.vendorEmail}
              </p>
            </div>)}
          {(invoice.supplier?.phone || invoice.vendorPhone) && (<div>
              <p className="text-sm text-gray-500">電話番号</p>
              <p className="mt-1 text-sm font-medium text-gray-900">
                {invoice.supplier?.phone || invoice.vendorPhone}
              </p>
            </div>)}
          {(invoice.supplier?.address1 || invoice.vendorAddress) && (<div className="md:col-span-3">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tfoot className="bg-gray-50">
              {invoice.previousBalance !== undefined && (<>
                  ...
                  {invoice.currentPayment !== undefined && (<tr>
                      <td colSpan={4} className="px-6 py-3 text-sm text-gray-600 text-right">
                        前回入金額
                      </td>
                      <td className="px-6 py-3 text-sm font-medium text-gray-900">
                  ...
                  {invoice.carryoverAmount !== undefined && (<tr>
                  ...
                  {invoice.currentSales !== undefined && (<tr>
                      <td colSpan={4} className="px-6 py-3 text-sm text-gray-600 text-right">
                        今月の請求額
                      </td>
                      <td className="px-6 py-3 text-sm font-medium text-gray-900">
              ...
              {invoice.currentInvoiceAmount !== undefined && (<tr className="bg-blue-50 border-t-2 border-blue-400">
      ...
      {/* 備考 */}
      {invoice.notes && (<div className="bg-white shadow rounded-lg p-6">
      ...
      {/* OCR元ファイル */}
      {(invoice.ocrResultId || invoice.fileId) && (<div className="bg-white shadow rounded-lg p-6">
          <h2 className="text-lg font-medium text-gray-900 mb-4">OCR元ファイル</h2>
          <div className="flex items-center gap-4">
            <div className="flex-1">
              <p className="text-sm text-gray-600 mb-2">
                この請求書はOCRによって自動生成されました。
              </p>
              <div className="flex gap-2">
                {invoice.fileId && (<>
                    <a href={`/api/documents/${invoice.fileId}/download`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors cursor-pointer">
                    ...
                    <a href={`/api/documents/${invoice.fileId}/download`} download className="inline-flex items-center gap-2 px-4 py-2 bg-gray-50 text-gray-700 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer">
                      <lucide_react_1.Download size={16}/>
                      ダウンロード
                    </a>
                  </>)}
                {invoice.ocrResultId && (<a href={`/api/ocr-results/${invoice.ocrResultId}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-gray-50 text-gray-700 rounded-lg hover:bg-gray-100 transition-colors">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* 振込先情報（仕入先から取得） */}
      {invoice.supplier?.bankTransferInfo && invoice.supplier.bankTransferInfo.length > 0 && (<div className="bg-white shadow rounded-lg p-6">
          <h2 className="text-lg font-medium text-gray-900 mb-4">振込先情報</h2>
          <div className="space-y-4">
            {invoice.supplier.bankTransferInfo.map((bank, index) => (<div key={index} className="border-b border-gray-200 pb-4 last:border-0">
      ...
      <div className="bg-white shadow rounded-lg p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => setShowPaymentModal(true)} className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 flex items-center space-x-2" disabled={invoice.paymentStatus === 'paid'}>
      ...
      <div className="bg-white shadow rounded-lg p-6">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={() => handleStatusUpdate('received')} disabled={updating || invoice.status === 'received'} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50">
            受領済みにする
          </button>
          <button onClick={() => handleStatusUpdate('approved')} disabled={updating || invoice.status === 'approved'} className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 disabled:opacity-50">
            承認する
          </button>
          <button onClick={() => handleStatusUpdate('cancelled')} disabled={updating || invoice.status === 'cancelled'} className="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 disabled:opacity-50">
            キャンセル
          </button>
        </div>
      </div>
      ...
      {/* 支払情報モーダル */}
      {showPaymentModal && (<div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={paymentData.paymentStatus} onChange={(e) => setPaymentData({ ...paymentData, paymentStatus: e.target.value })} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
              ...
              <div>
                ...
                <input type="number" value={paymentData.paidAmount} onChange={(e) => setPaymentData({ ...paymentData, paidAmount: Number(e.target.value) })} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"/>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">支払日</label>
                <input type="date" value={paymentData.paidDate} onChange={(e) => setPaymentData({ ...paymentData, paidDate: e.target.value })} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"/>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">支払参照番号</label>
                <input type="text" value={paymentData.paymentReference} onChange={(e) => setPaymentData({ ...paymentData, paymentReference: e.target.value })} placeholder="振込番号など" className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"/>
              </div>
            </div>
            <div className="mt-6 flex justify-end space-x-3">
              <button onClick={() => setShowPaymentModal(false)} className="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
                キャンセル
              </button>
              <button onClick={handlePaymentUpdate} disabled={updating} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50">
                更新
              </button>
            </div>
          </div>
