...
exports.customerTools = exports.generateCustomerReportTool = exports.calculateCustomerLifetimeValueTool = exports.analyzeCustomerTool = exports.searchCustomersTool = exports.updateCustomerTool = exports.createCustomerTool = void 0;
const mongodb_client_1 = require("@/lib/mongodb-client");
const logger_1 = require("@/lib/logger");
exports.createCustomerTool = {
    ...
    handler: async (params) => {
        logger_1.logger.info('Creating customer via Mastra:', params);
...
exports.updateCustomerTool = {
    ...
    handler: async (params) => {
        logger_1.logger.info('Updating customer via Mastra:', params);
        const db = await (0, mongodb_client_1.getDatabase)();
        const collection = db.collection('customers');
        const updates = {
            updatedAt: new Date()
        ...
        if (params.updates.name)
            updates.companyName = params.updates.name;
        if (params.updates.email)
            updates.email = params.updates.email;
        if (params.updates.phone)
            updates.phone = params.updates.phone;
        if (params.updates.credit_limit !== undefined)
            updates.creditLimit = params.updates.credit_limit;
        if (params.updates.status)
            updates.isActive = params.updates.status === 'active';
        ...
        const result = await collection.updateOne({ customerId: params.customer_id }, { $set: updates });
...
exports.searchCustomersTool = {
    ...
    handler: async (params) => {
        logger_1.logger.info('Searching customers via Mastra:', params);
        const db = await (0, mongodb_client_1.getDatabase)();
        const collection = db.collection('customers');
        const query = {};
        if (params.query) {
...
exports.analyzeCustomerTool = {
    ...
    handler: async (params) => {
        logger_1.logger.info('Analyzing customer via Mastra:', params);
        const db = await (0, mongodb_client_1.getDatabase)();
        ...
        switch (params.analysis_type) {
            ...
            case 'payment_behavior':
                ...
                const overdueInvoices = invoices.filter(inv => inv.status === 'unpaid' && new Date(inv.dueDate) < new Date());
...
exports.calculateCustomerLifetimeValueTool = {
    ...
    handler: async (params) => {
        logger_1.logger.info('Calculating customer lifetime value via Mastra:', params);
        const db = await (0, mongodb_client_1.getDatabase)();
        const customer = await db.collection('customers').findOne({ customerId: params.customer_id });
        if (!customer) {
            throw new Error(`顧客ID ${params.customer_id} が見つかりません`);
        ...
        const firstInvoiceDate = invoices.reduce((min, inv) => inv.issueDate < min ? inv.issueDate : min, invoices[0]?.issueDate || new Date());
        const lastInvoiceDate = invoices.reduce((max, inv) => inv.issueDate > max ? inv.issueDate : max, invoices[0]?.issueDate || new Date());
...
exports.generateCustomerReportTool = {
    ...
    handler: async (params) => {
        logger_1.logger.info('Generating customer report via Mastra:', params);
        const db = await (0, mongodb_client_1.getDatabase)();
        ...
        switch (params.report_type) {
            case 'summary':
                ...
                reportData.summary = {
                    ...
                    new_customers_period: customers.filter(c => c.createdAt >= startDate && c.createdAt <= endDate).length
                };
                break;
            case 'top_customers':
                const invoices = await db.collection('invoices').find({
