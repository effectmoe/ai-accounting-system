...
export class ToolManager {
  ...
  async loadGroup(groupName: string) {
    ...
    try {
      ...
      logger.debug(`Loaded tool group: ${groupName}`);
    } catch (error) {
      logger.error(`Failed to load tool group ${groupName}:`, error);
    }
  }
  ...
  async unloadGroup(groupName: string) {
    ...
    try {
      ...
      logger.error(`Failed to unload tool group ${groupName}:`, error);
    }
  }
  ...
  async getGroupTools(groupName: string): Promise<string[]> {
    try {
      ...
      logger.error(`Failed to get tools for group ${groupName}:`, error);
      return [];
    }
  }
} 