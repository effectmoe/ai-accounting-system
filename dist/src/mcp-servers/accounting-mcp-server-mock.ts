...
/**
 * Accounting MCP Server (Mock Version)
 * MongoDBなしで動作するモック版
 */
...
class AccountingMCPServer {
  ...
  private async generateReport(data: any) {
    // モックデータから該当期間のエントリを抽出
    const entries = mockData.journalEntries.filter(entry => {
      if (entry.companyId !== data.companyId) return false;
  ...
  async run() {
    ...
    logger.error('Accounting MCP Server (Mock) running on stdio');
  }
}
...
if (require.main === module) {
  ...
  server.run().catch((error) => {
    logger.error('Failed to run Accounting MCP Server:', error);
    process.exit(1);
  });
}
