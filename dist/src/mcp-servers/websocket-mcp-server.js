...
class WebSocketMCPServer {
    ...
    async handleStartServer(args) {
        const { port } = args;
        try {
            const wsManager = await (0, websocket_manager_1.startWebSocketServer)();
    ...
    async handleSendProgress(args) {
        const { sessionId, operation, progress, currentStep, totalSteps, completedSteps, status, data, error, } = args;
    ...
    async handleBroadcastToAll(args) {
    ...
    async run() {
        ...
        logger_1.logger.error('WebSocket MCP Server running on stdio');
    }
}
exports.WebSocketMCPServer = WebSocketMCPServer;
if (require.main === module) {
    const server = new WebSocketMCPServer();
    server.run().catch((error) => {
        logger_1.logger.error('Failed to run WebSocket MCP Server:', error);
        process.exit(1);
    });
}
//# sourceMappingURL=websocket-mcp-server.js.map