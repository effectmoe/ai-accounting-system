...
class MLAnalyticsMCPServer {
    ...
    async handleQuickAnalysis(args) {
        ...
        const values = data.flatMap((item) => Object.values(item.features));
        const basicStats = {
            count: values.length,
            ...
            content: [
                {
                    ...
                    text: JSON.stringify({
                        ...
                        basicStats,
                        anomalyDetection: {
                            anomaliesFound: anomalyResult.results.anomalies?.filter((a) => a.isAnomaly).length || 0,
            ...
            content: [
                {
                    ...
                    text: JSON.stringify({
                        ...
                        threshold,
                        method,
                        anomalies: detectedAnomalies.slice(0, 10),
                        insights: result.insights,
                        recommendations: result.recommendations,
                    }, null, 2),
                },
    ...
    async run() {
        ...
        logger_1.logger.error('ML Analytics MCP Server running on stdio');
    }
}
exports.MLAnalyticsMCPServer = MLAnalyticsMCPServer;
if (require.main === module) {
    const server = new MLAnalyticsMCPServer();
    server.run().catch((error) => {
        logger_1.logger.error('Failed to run ML Analytics MCP Server:', error);
        process.exit(1);
    });
}
//# sourceMappingURL=ml-analytics-mcp-server.js.map