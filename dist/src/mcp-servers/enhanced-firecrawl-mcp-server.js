...
class EnhancedFirecrawlMCPServer {
    ...
    async handleAnalyzeCompetitor(args) {
        const { url } = args;
        ...
        extractionTargets.forEach((target) => {
            switch (target) {
                ...
                case 'services':
                    ...
                    const productLines = content.split('\n').filter(line => productKeywords.some(keyword => line.toLowerCase().includes(keyword)));
    ...
    async handleCompetitiveAnalysis(args) {
        ...
        const analysisPromises = competitorUrls.map(async (url) => {
            try {
        ...
        const comparison = {};
        analysisAspects.forEach((aspect) => {
            switch (aspect) {
                case 'pricing':
    ...
    async run() {
        ...
        logger_1.logger.error('Enhanced Firecrawl MCP Server running on stdio');
    }
}
exports.EnhancedFirecrawlMCPServer = EnhancedFirecrawlMCPServer;
if (require.main === module) {
    const server = new EnhancedFirecrawlMCPServer();
    server.run().catch((error) => {
        logger_1.logger.error('Failed to run Enhanced Firecrawl MCP Server:', error);
        process.exit(1);
    });
}
//# sourceMappingURL=enhanced-firecrawl-mcp-server.js.map