...
function AIChatDialog({ isOpen, onClose, onComplete, onDataApply, initialInvoiceData, mode = 'create', companyId, existingConversationId, invoiceId, documentType = 'invoice', title, placeholder }) {
    ...
    (0, react_1.useEffect)(() => {
        if (process.env.NODE_ENV === 'development') {
            logger_1.logger.debug('[AIChatDialog] 音声認識デバッグ情報:', {
        ...
        if (isOpen) {
            ...
            if (messages.length === 0) {
                ...
                if (mode === 'edit' && initialInvoiceData) {
                    ...
                    logger_1.logger.debug('[AIChatDialog] Setting initial data for edit mode:', completeInitialData);
                ...
                if (!conversationId) {
                    ...
                    logger_1.logger.debug('[AIChatDialog] New conversation started with ID:', newConversationId);
                    logger_1.logger.debug('[AIChatDialog] existingConversationId:', existingConversationId);
                }
                else {
                    logger_1.logger.debug('[AIChatDialog] Using existing conversation ID:', conversationId);
                    logger_1.logger.debug('[AIChatDialog] Normalized from:', existingConversationId, 'to:', conversationId);
                }
            }
        }
    ...
    (0, react_1.useEffect)(() => {
        logger_1.logger.debug('[Frontend] currentInvoiceData changed:', currentInvoiceData);
    }, [currentInvoiceData]);
    // カーソル位置を保存・取得する関数
    ...
    (0, react_1.useEffect)(() => {
        if (transcript) {
            ...
            logger_1.logger.debug('[SpeechRecognition] Transcript inserted at cursor position:', cursorPosition);
    ...
    const sendMessage = async (customInput) => {
        ...
        try {
            ...
            logger_1.logger.debug('[Frontend] Sending data to API:', {
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    logger_1.logger.error('[Frontend] Error response:', errorData);
                    ...
                    logger_1.logger.error('[Frontend] Failed to parse error response');
            ...
            logger_1.logger.debug('[Frontend] Received response from API:', {
            ...
            // 請求書データを更新
            if (result.data) {
                logger_1.logger.debug('[Frontend] Updating invoice data:', result.data);
                logger_1.logger.debug('[Frontend] Data details:', {
                ...
                if (result.data.items && result.data.items.length > 0) {
                    logger_1.logger.debug('[Frontend] Items detail:');
                    result.data.items.forEach((item, index) => {
                        logger_1.logger.debug(`[Frontend] Item ${index}:`, {
                ...
                // データが正しく設定されているか確認
                if (result.data.items && result.data.items.length > 0) {
                    logger_1.logger.debug('[Frontend] Items detected, updating currentInvoiceData');
                }
                if (result.data.customerName) {
                    logger_1.logger.debug('[Frontend] Customer name detected:', result.data.customerName);
                }
                // 完全なデータ構造で更新（既存データとマージ）
                setCurrentInvoiceData(prev => {
                    logger_1.logger.debug('[Frontend] Previous state:', prev);
                    logger_1.logger.debug('[Frontend] Result data from backend:', result.data);
                    logger_1.logger.debug('[Frontend] Customer name in result:', result.data.customerName);
                    ...
                    logger_1.logger.debug('[Frontend] New state will be:', newData);
                    logger_1.logger.debug('[Frontend] Customer name updated to:', newData.customerName);
                    logger_1.logger.debug('[Frontend] Date fields updated:', {
                        invoiceDate: newData.invoiceDate,
                        dueDate: newData.dueDate
                    });
                    logger_1.logger.debug('[Frontend] Items update:', {
                    ...
                    if (newData.items && newData.items.length > 0) {
                        newData.items.forEach((item, index) => {
                            logger_1.logger.debug(`[Frontend] Item ${index} details:`, {
                                description: item.description,
                                quantity: item.quantity,
                                unitPrice: item.unitPrice,
                                amount: item.amount,
                ...
                logger_1.logger.debug('[Frontend] No data in result, not updating currentInvoiceData');
            }
        }
        catch (error) {
    ...
    const completeConversation = () => {
        if (currentInvoiceData && (currentInvoiceData.customerName || (currentInvoiceData.items && currentInvoiceData.items.length > 0))) {
            logger_1.logger.debug('[Frontend] Completing conversation with data:', JSON.parse(JSON.stringify(currentInvoiceData)));
            logger_1.logger.debug('[Frontend] Final data details:', {
            ...
            if (currentInvoiceData.items && currentInvoiceData.items.length > 0) {
                logger_1.logger.debug('[Frontend] Final items breakdown:');
                currentInvoiceData.items.forEach((item, index) => {
                    logger_1.logger.debug(`[Frontend] Final Item ${index}:`, {
                        description: item.description,
                        quantity: item.quantity,
                        unitPrice: item.unitPrice,
                        amount: item.amount,
            ...
            logger_1.logger.debug('[Frontend] Complete data to be passed:', completeData);
        ...
        {currentInvoiceData && (currentInvoiceData.customerName ||
            ...
            <div className="text-sm text-blue-800 space-y-1">
              ...
              {(() => {
                ...
                logger_1.logger.debug('[Frontend] Display calculation:', { subtotal, taxAmount, totalAmount });
    ...
    {placeholder || `AI${documentType === 'quote' ? '見積書' : '請求書'}アシスタントに質問や指示を入力してください`}
     ...
     title="会話ログをダウンロード">
            ...
            e.preventDefault();
            // 保存ボタンのクリック状態を確認
            logger_1.logger.debug('[AIChatDialog] Save button clicked');
            logger_1.logger.debug('[AIChatDialog] Document type:', documentType);
            logger_1.logger.debug('[AIChatDialog] Current data:', currentInvoiceData);
            logger_1.logger.debug('[AIChatDialog] Data validation:', {
            ...
            // 各アイテムの詳細もログ出力
            if (currentInvoiceData?.items && currentInvoiceData.items.length > 0) {
                logger_1.logger.debug('[AIChatDialog] Items details:');
                currentInvoiceData.items.forEach((item, index) => {
                    logger_1.logger.debug(`[AIChatDialog] Item ${index}:`, {
                        description: item.description,
                        quantity: item.quantity,
                        unitPrice: item.unitPrice,
                        amount: item.amount,
            ...
            if (companyId && conversationId && messages.length > 1) {
                ...
                if (!conversationId.startsWith('conv_')) {
                    logger_1.logger.warn('[AIChatDialog] Invalid conversationId format:', conversationId);
                    finalConversationId = `conv_${conversationId}`;
                    setConversationId(finalConversationId);
                    logger_1.logger.debug('[AIChatDialog] Corrected conversationId to:', finalConversationId);
                }
                try {
                    logger_1.logger.debug('[AIChatDialog] Saving conversation with ID:', finalConversationId);
                    logger_1.logger.debug('[AIChatDialog] Message count:', messages.length);
                    logger_1.logger.debug('[AIChatDialog] Current sessionId:', sessionId);
                    logger_1.logger.debug('[AIChatDialog] existingConversationId prop:', existingConversationId);
                    // 全てのメッセージを含めて保存
                    ...
                    logger_1.logger.debug('[AIChatDialog] Saving conversation with data:', {
                    ...
                    if (!response.ok) {
                        logger_1.logger.error('会話履歴の保存に失敗しました');
                    }
                    else {
                        const result = await response.json();
                        logger_1.logger.debug('[AIChatDialog] Conversation saved successfully:', result);
                        logger_1.logger.debug('[AIChatDialog] Saved conversation ID:', finalConversationId);
                        logger_1.logger.debug('[AIChatDialog] Total messages saved:', result.messagesCount || messages.length);
                        // 保存が成功した場合、データに会話IDを追加
                        currentInvoiceData.aiConversationId = finalConversationId;
                    }
                }
                catch (error) {
                    logger_1.logger.error('会話履歴保存エラー:', error);
                }
            }
            logger_1.logger.debug('[AIChatDialog] Calling completeConversation');
