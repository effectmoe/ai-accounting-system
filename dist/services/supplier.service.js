...
class SupplierService {
    ...
    static async getSuppliers(params) {
        const { status, search, page = 1, limit = 20, sortBy = 'companyName', sortOrder = 'asc' } = params || {};
    ...
    static async getSupplierById(id) {
        logger_1.logger.debug('===== [SupplierService] getSupplierById Debug START =====');
        logger_1.logger.debug('[1] Looking for supplier with ID:', id);
        const db = await (0, mongodb_client_1.getDatabase)();
        const collection = db.collection(COLLECTION_NAME);
        const supplier = await collection.findOne({ _id: new mongodb_1.ObjectId(id) });
        if (!supplier) {
            logger_1.logger.debug('[2] Supplier not found in database');
            throw new Error('Supplier not found');
        }
        logger_1.logger.debug('[2] Raw supplier from MongoDB:', JSON.stringify({
            _id: supplier._id,
        ...
        logger_1.logger.debug('[3] MongoDB field types:', {
        ...
        logger_1.logger.debug('[4] Final supplier object to return:', JSON.stringify({
            _id: result._id,
            id: result.id,
            phone: result.phone,
            fax: result.fax,
        ...
        logger_1.logger.debug('===== [SupplierService] getSupplierById Debug END =====');
        return result;
    }
    // 仕入先作成
    ...
    static async updateSupplier(id, data) {
        ...
        if (data.bankAccountId) {
            updateData.bankAccountId = new mongodb_1.ObjectId(data.bankAccountId);
        }
    ...
    static async deleteSupplier(id) {
        ...
        const [purchaseOrders, supplierQuotes] = await Promise.all([
            db.collection('purchaseOrders').countDocuments({ supplierId: new mongodb_1.ObjectId(id) }),
            db.collection('supplierQuotes').countDocuments({ supplierId: new mongodb_1.ObjectId(id) })
        ]);
    ...
    static async getSupplierStats(supplierId) {
        const db = await (0, mongodb_client_1.getDatabase)();
        const supplierObjectId = new mongodb_1.ObjectId(supplierId);
        // 発注書の統計
