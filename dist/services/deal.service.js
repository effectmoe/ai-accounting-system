...
class DealService {
    ...
    static async getDeals(params) {
        const { status, dealType, customerId, search, page = 1, limit = 20, sortBy = 'startDate', sortOrder = 'desc', dateRange } = params || {};
    ...
    static async createDeal(data) {
        ...
        const deal = {
            ...data,
            dealNumber,
            customerId: new mongodb_1.ObjectId(data.customerId),
    ...
    static async updateDeal(id, data) {
        ...
        const collection = db.collection(COLLECTION_NAME);
        // 現在の案件を取得
        const currentDeal = await collection.findOne({ _id: new mongodb_1.ObjectId(id) });
        if (!currentDeal) {
            throw new Error('Deal not found');
        }
        const updateData = {
        ...
        // ObjectId型への変換
        if (data.customerId) {
            updateData.customerId = new mongodb_1.ObjectId(data.customerId);
        }
        if (data.relatedQuotes) {
            updateData.relatedQuotes = data.relatedQuotes.map(id => new mongodb_1.ObjectId(id));
        }
        if (data.relatedInvoices) {
            updateData.relatedInvoices = data.relatedInvoices.map(id => new mongodb_1.ObjectId(id));
        }
    ...
    static async deleteDeal(id) {
        const result = await mongodb_client_1.db.delete(COLLECTION_NAME, id);
        if (!result) {
            throw new Error('Deal not found');
        }
        return { deleted: true };
    ...
    // 案件アクティビティ追加
    static async addActivity(dealId, activity) {
        const client = await (0, mongodb_client_1.getMongoClient)();
        const db = client.db(DB_NAME);
        const collection = db.collection(COLLECTION_NAME);
        const newActivity = {
        ...
        const result = await collection.findOneAndUpdate({ _id: new mongodb_1.ObjectId(dealId) }, {
