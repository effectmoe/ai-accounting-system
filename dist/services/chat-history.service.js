...
class ChatHistoryService {
    ...
    constructor() {
        ...
        if (!connectionString) {
            logger_1.logger.error('[ChatHistoryService] MONGODB_URI環境変数が設定されていません');
            throw new Error('MONGODB_URI環境変数が必要です');
        }
        logger_1.logger.debug('[ChatHistoryService] MongoDB接続を初期化中...');
    ...
    async connect() {
        if (!this.isConnected) {
            try {
                ...
                logger_1.logger.debug('[ChatHistoryService] MongoDBに接続成功');
            }
            catch (error) {
                logger_1.logger.error('[ChatHistoryService] MongoDB接続エラー:', error);
                throw error;
            }
        }
    }
    ...
    async getSession(sessionId) {
        await this.connect();
        logger_1.logger.debug(`[ChatHistoryService] セッション取得: ${sessionId}`);
        ...
        if (!session) {
            try {
                ...
                if (session) {
                    logger_1.logger.debug(`[ChatHistoryService] _idで検索してセッション発見: ${sessionId}`);
                }
            }
            catch (e) {
                logger_1.logger.debug(`[ChatHistoryService] ${sessionId}はObjectIdとして無効`);
            }
        }
        else {
            logger_1.logger.debug(`[ChatHistoryService] sessionIdで検索してセッション発見: ${sessionId}`);
        }
        if (!session) {
            logger_1.logger.debug(`[ChatHistoryService] セッションが見つかりません: ${sessionId}`);
        }
        return session;
    }
    /**
    ...
    async getUserSessions(userId, options = {}) {
        ...
        logger_1.logger.debug('[ChatHistoryService] セッション検索フィルター:', JSON.stringify(filter));
        const sortOption = {};
        ...
        logger_1.logger.debug(`[ChatHistoryService] 検索結果: ${sessions.length}件のセッション (全${total}件)`);
