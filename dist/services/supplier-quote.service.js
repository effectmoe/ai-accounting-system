...
class SupplierQuoteService {
    ...
    async searchSupplierQuotes(params) {
        try {
            ...
            logger_1.logger.error('Error in searchSupplierQuotes:', error);
    ...
    async createSupplierQuote(quoteData) {
        try {
            ...
            if (existing) {
                logger_1.logger.debug(`[SupplierQuoteService] Duplicate quote number detected: ${quoteData.quoteNumber}`);
                // 重複が検出された場合、新しい番号を生成
                finalQuoteNumber = await this.generateSupplierQuoteNumber();
                logger_1.logger.debug(`[SupplierQuoteService] Generated new quote number: ${finalQuoteNumber}`);
            }
            // 仕入先の存在確認
            ...
            logger_1.logger.error('Error in createSupplierQuote:', error);
    ...
    async getSupplierQuote(id) {
        try {
            logger_1.logger.debug('[SupplierQuoteService] getSupplierQuote called with ID:', id);
            // IDの検証
            if (!id || id === 'undefined' || id === 'null') {
                logger_1.logger.debug('[SupplierQuoteService] Invalid ID provided:', id);
                return null;
            }
            // IDの長さチェック（ObjectIdは24文字）
            if (id.length !== 24) {
                logger_1.logger.debug('[SupplierQuoteService] Invalid ObjectId length:', id.length);
                return null;
            }
            const quote = await mongodb_client_1.db.findById(this.collectionName, id);
            if (!quote) {
                logger_1.logger.debug('[SupplierQuoteService] No supplier quote found for ID:', id);
                return null;
            }
            logger_1.logger.debug('[SupplierQuoteService] Supplier quote found:', {
                _id: quote._id,
            ...
            logger_1.logger.error('Error in getSupplierQuote:', error);
    ...
    async updateSupplierQuote(id, updateData) {
        try {
            logger_1.logger.debug('[SupplierQuoteService] updateSupplierQuote called with:', {
                id,
            ...
            logger_1.logger.error('Error in updateSupplierQuote:', error);
    ...
    async deleteSupplierQuote(id) {
        try {
            ...
            logger_1.logger.error('Error in deleteSupplierQuote:', error);
    ...
    async updateSupplierQuoteStatus(id, status, statusDate) {
        try {
            ...
            logger_1.logger.error('Error in updateSupplierQuoteStatus:', error);
    ...
    async generateSupplierQuoteNumber() {
        try {
            ...
            const existingQuotes = await mongodb_client_1.db.find(this.collectionName, {
            ...
            logger_1.logger.warn('Failed to generate unique quote number after 100 attempts');
            const timestamp = Date.now().toString().slice(-6);
            return `${prefix}-${seq.toString().padStart(3, '0')}-${timestamp}`;
        }
        catch (error) {
            logger_1.logger.error('Error in generateSupplierQuoteNumber:', error);
