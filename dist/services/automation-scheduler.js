...
class AutomationScheduler {
    ...
    async runTask(id) {
        ...
        try {
            ...
            switch (task.id) {
                case 'ocr-receipts':
                    logger_1.logger.debug('OCR処理を実行中...');
                    break;
                case 'invoice-generation':
                    logger_1.logger.debug('請求書生成を実行中...');
                    break;
                case 'tax-calculation':
                    logger_1.logger.debug('税務計算を実行中...');
                    break;
                case 'report-generation':
                    logger_1.logger.debug('レポート生成を実行中...');
                    break;
                case 'backup-data':
                    logger_1.logger.debug('データバックアップを実行中...');
                    break;
            }
