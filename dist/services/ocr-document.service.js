...
exports.OCRDocumentService = void 0;
const mongodb_1 = require("mongodb");
const mongodb_client_1 = require("@/lib/mongodb-client");
const account_category_ai_1 = require("@/lib/account-category-ai");
...
class OCRDocumentService {
    ...
    accountCategoryAI;
    constructor(config = {}) {
        this.db = mongodb_client_1.DatabaseService.getInstance();
    ...
    async createDocument(request) {
        try {
            (0, ocr_utils_1.logOCRProcessing)('Start', request, {
                hasAIData: !!request.aiStructuredData,
                hasOCRResultId: !!request.ocrResultId
            });
            // Determine processing method
            if (request.aiStructuredData) {
                return await this.createFromAIStructuredData(request.aiStructuredData, request.companyId || this.config.defaultCompanyId);
            }
            ...
            logger_1.logger.error('OCR document creation failed:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async createFromAIStructuredData(aiData, companyId) {
        (0, ocr_utils_1.logOCRProcessing)('AI Processing', aiData, {
        ...
        const documentNumber = (0, ocr_utils_1.generateDocumentNumber)(documentType, aiData.documentNumber);
        // Prepare field copy configuration
        const fieldCopyConfig = {
        ...
        const baseFields = (0, ocr_utils_1.copyAllFields)(aiData, {}, fieldCopyConfig);
        // Build comprehensive notes
        const notes = (0, ocr_utils_1.buildComprehensiveNotes)(aiData, ['AI駆動のOCR解析により作成']);
        ...
        const documentData = {
            ...
            partnerName: (0, ocr_utils_1.extractPartnerName)(aiData),
        ...
        // Save document
        const savedDoc = await this.db.create(mongodb_client_1.Collections.DOCUMENTS, documentData);
        // Save items
     ...
     */
    async createFromOCRResult(request) {
        const { ocrResultId, approvedBy } = request;
        if (!ocrResultId) {
            throw new Error('OCR Result ID is required');
        }
        // Fetch OCR result
        const ocrResult = await this.db.findById(mongodb_client_1.Collections.OCR_RESULTS, ocrResultId);
        if (!ocrResult) {
            throw new Error('OCR result not found');
        }
        (0, ocr_utils_1.logOCRProcessing)('OCR Result Processing', ocrResult, {
        ...
        const validatedData = (0, ocr_utils_1.validateOCRData)(ocrData);
        // Prepare document data
        ...
        const documentData = {
            ...
            documentType,
            type: documentType,
            documentNumber: (0, ocr_utils_1.generateDocumentNumber)(documentType),
            displayNumber: '',
            issueDate: validatedData.issueDate || new Date().toISOString().split('T')[0],
            partnerName: (0, ocr_utils_1.extractPartnerName)(validatedData),
            ...
            subtotal: (0, ocr_utils_1.calculateSubtotal)(validatedData.totalAmount || 0, validatedData.taxAmount || 0, validatedData.subtotalAmount),
            taxAmount: validatedData.taxAmount || 0,
            totalAmount: validatedData.totalAmount || 0,
            status: 'pending',
            notes: (0, ocr_utils_1.buildComprehensiveNotes)(validatedData, [
     ...
     */
    async createFromSimpleData(request) {
        (0, ocr_utils_1.logOCRProcessing)('Simple Processing', request);
        const validatedData = (0, ocr_utils_1.validateOCRData)(request);
        const documentType = (request.document_type || 'receipt');
        const companyId = request.companyId || this.config.defaultCompanyId;
        // Calculate financial values
        const subtotal = (0, ocr_utils_1.calculateSubtotal)(validatedData.totalAmount || 0, validatedData.taxAmount || 0, validatedData.subtotalAmount);
        // Build notes
        const notes = (0, ocr_utils_1.buildComprehensiveNotes)(validatedData, ['OCRデータより作成']);
        // Prepare document data
        const documentData = {
            companyId,
            documentType,
            type: documentType,
            documentNumber: (0, ocr_utils_1.generateDocumentNumber)(documentType, validatedData.receiptNumber || validatedData.documentNumber),
            displayNumber: '',
        ...
        if (this.config.enableAccountPrediction && request.extracted_text) {
            this.predictAccountCategoryAsync(savedDoc._id, {
    ...
    async saveDocumentItems(documentId, items) {
        const itemPromises = items.map((item, index) => {
            ...
            return this.db.create(mongodb_client_1.Collections.ITEMS, mongoItem);
        });
     ...
     */
    async updateOCRResultStatus(ocrResultId, documentId) {
        try {
            await this.db.updateById(mongodb_client_1.Collections.OCR_RESULTS, ocrResultId, {
                linkedDocumentId: documentId,
                status: 'processed',
                updatedAt: new Date()
            });
            logger_1.logger.debug(`OCR result ${ocrResultId} updated successfully`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to update OCR result ${ocrResultId}:`, error);
     ...
     */
    async predictAccountCategoryAsync(documentId, ocrData, companyId) {
        try {
            if (!this.accountCategoryAI) {
                this.accountCategoryAI = new account_category_ai_1.AccountCategoryAI();
            }
            const prediction = await this.accountCategoryAI.predictAccountCategory(ocrData, companyId);
            if (prediction && prediction.confidence >= this.config.confidenceThreshold) {
                await this.db.updateById(mongodb_client_1.Collections.DOCUMENTS, documentId.toString(), {
                ...
                logger_1.logger.debug(`Account category predicted for document ${documentId}`);
            }
        }
        catch (error) {
            logger_1.logger.error(`Account category prediction failed for document ${documentId}:`, error);
     ...
     */
    getSuccessMessage(documentType, receiptType) {
        const typeLabels = {
            receipt: '領収書',
            invoice: '請求書',
            estimate: '見積書',
...
exports.OCRDocumentService = OCRDocumentService;
