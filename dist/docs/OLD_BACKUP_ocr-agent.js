...
exports.ocrAgent = (0, core_1.createAgent)({
    ...
    tools: {
        ...
        handwritingOCR: {
            ...
            execute: async ({ fileData, language }) => {
                try {
                    ...
                    const response = await (0, node_fetch_1.default)('https://api.handwritingocr.com/v1/ocr', {
                        method: 'POST',
                        headers: {
        ...
        callMCPServer: {
            ...
            execute: async ({ server, method, params }) => {
                // MCP サーバーとの通信
                const response = await (0, node_fetch_1.default)(`http://localhost:3002/mcp/${server}`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    execute: async ({ input, tools }) => {
        try {
            ...
            // ファイルデータの取得
            if (input.filePath) {
                fileData = fs_1.default.readFileSync(input.filePath);
                fileName = path_1.default.basename(input.filePath);
            }
            else if (input.fileUrl) {
                const response = await (0, node_fetch_1.default)(input.fileUrl);
                fileData = Buffer.from(await response.arrayBuffer());
                fileName = path_1.default.basename(input.fileUrl);
            }
            else {
