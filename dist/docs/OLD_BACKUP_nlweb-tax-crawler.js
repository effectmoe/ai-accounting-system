...
exports.TaxInfoCrawler = void 0;
exports.runTaxCrawler = runTaxCrawler;
exports.searchTaxInfoByQuery = searchTaxInfoByQuery;
const nlweb_1 = require("@microsoft/nlweb");
...
class TaxInfoCrawler {
    ...
    async initialize() {
        ...
        this.nlweb = new nlweb_1.NLWebDriver(page);
    }
    async close() {
        if (this.browser) {
    ...
    // セクションごとのクロール
    async crawlSection(source, section) {
        try {
    ...
    async extractContent() {
        ...
        const content = await page.evaluate(() => {
            ...
            tables.forEach(table => {
                const rows = Array.from(table.querySelectorAll('tr')).map(tr => Array.from(tr.querySelectorAll('td, th')).map(cell => cell.textContent?.trim() || ''));
    ...
    calculatePriority(url) {
        if (url.includes('改正') || url.includes('新着'))
            return 10;
        if (url.includes('インボイス'))
            return 9;
        if (url.includes('手続き'))
            return 8;
        return 5;
...
exports.TaxInfoCrawler = TaxInfoCrawler;
// クローラーの実行
async function runTaxCrawler() {
...
// 特定のクエリで税制情報を検索
async function searchTaxInfoByQuery(query) {
    const crawler = new TaxInfoCrawler();
    try {
        await crawler.initialize();
