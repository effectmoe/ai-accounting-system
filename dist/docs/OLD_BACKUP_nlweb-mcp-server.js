...
const nlweb_tax_crawler_1 = require("./nlweb-tax-crawler");
// Supabase設定
...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        case 'search_tax_info': {
            try {
                ...
                if (!dbResults || dbResults.length === 0) {
                    const crawlResults = await (0, nlweb_tax_crawler_1.searchTaxInfoByQuery)(args.query);
                    return {
                        content: [
                            {
                                type: 'text',
        ...
        case 'crawl_tax_info': {
            try {
                // バックグラウンドでクロール開始
                (0, nlweb_tax_crawler_1.runTaxCrawler)().catch(console.error);
                return {
                    content: [
                        {
                            type: 'text',
