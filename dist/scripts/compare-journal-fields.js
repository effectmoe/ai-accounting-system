...
async function analyzeJournalsCollection(db, journalId, receiptId) {
    console.log('\n--- journalsコレクションの分析 ---');
    ...
    if (!journal) {
        try {
            journal = await journals.findOne({ _id: new mongodb_1.ObjectId(journalId) });
        }
        catch (e) { }
    }
    if (journal) {
...
async function analyzeDocumentsCollection(db, journalId, receiptId) {
...
async function findDocument(collection, id, expectedType) {
    // 文字列IDで検索
    let doc = await collection.findOne({ _id: id });
    // ObjectIdで検索
    if (!doc) {
        try {
            doc = await collection.findOne({ _id: new mongodb_1.ObjectId(id) });
        }
        catch (e) { }
    }
    // タイプチェック
...
async function compareWithSourceDocument(db, journal, sourceDocumentId) {
    const documents = db.collection('documents');
...
function compareFields(source, journal) {
    console.log('\n📊 フィールド比較結果:');
    console.log('='.repeat(80));
    const results = {
        regular: { copied: 0, notCopied: 0, fields: [] },
        parking: { copied: 0, notCopied: 0, fields: [] }
    };
    // 通常フィールドの比較
    console.log('\n📋 通常フィールド:');
