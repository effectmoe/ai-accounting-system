...
async function fixParkingFieldMapping(dryRun = true) {
    ...
    try {
        ...
        for (const doc of documents) {
            ...
            if (Object.keys(updateData).length > 0) {
                ...
                if (!dryRun) {
                    try {
                        const updateResult = await documentsCollection.updateOne({ _id: doc._id }, {
                            $set: {
                                ...updateData,
                                updatedAt: new Date()
                            }
        ...
        logger_1.logger.error('Fix script error:', error);
    }
    finally {
        await client.close();
        console.log('\n✅ MongoDB接続を閉じました');
