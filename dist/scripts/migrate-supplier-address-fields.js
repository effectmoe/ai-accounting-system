    ...
    try {
        ...
        for (const supplier of suppliersWithOldAddress) {
            try {
                ...
                // Update the supplier
                const result = await collection.updateOne({ _id: supplier._id }, {
                    $set: updateFields,
        ...
        for (const supplier of suppliersWithoutNewFields) {
            try {
                ...
                if (!supplier.address1)
                    updateFields.address1 = supplier.address || '';
                if (!supplier.address2)
                    updateFields.address2 = '';
                if (!supplier.postalCode)
                    updateFields.postalCode = '';
                ...
                if (!supplier.website)
                    updateFields.website = '';
                const result = await collection.updateOne({ _id: supplier._id }, { $set: updateFields });
                if (result.modifiedCount > 0) {
