...
async function checkCategories() {
    ...
    try {
        ...
        // 最新の10件のドキュメントを取得
        const documents = await db.find(mongodb_client_1.Collections.DOCUMENTS, { companyId }, {
            limit: 10,
            sort: { createdAt: -1 }
        });
        ...
        // 統計情報
        const totalDocs = await db.count(mongodb_client_1.Collections.DOCUMENTS, { companyId });
        const categorizedDocs = await db.count(mongodb_client_1.Collections.DOCUMENTS, {
            companyId,
            category: { $exists: true, $ne: null }
        });
        const aiPredictedDocs = await db.count(mongodb_client_1.Collections.DOCUMENTS, {
            companyId,
        ...
        await mongodb_client_1.DatabaseService.close();
    }
}
checkCategories();
