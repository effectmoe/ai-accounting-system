...
function OCRUpload() {
            ...
            reader.onload = async (e) => {
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        // デモモードの場合は追加のメッセージ
                        if (result.demo) {
                            react_hot_toast_1.default.info('デモモード: モックデータが保存されました', { duration: 4000 });
                        }
                    }
                    else {
                    ...
                    react_hot_toast_1.default.error(`エラー: ${error}`);
                }
            };
            reader.readAsDataURL(file);
        }
        catch (error) {
            react_hot_toast_1.default.error(`${file.name} のアップロードに失敗: ${error}`);
        }
    };
    ...
    const saveToDatabase = async (data) => {
        try {
            ...
            if (response.ok) {
                react_hot_toast_1.default.success('データベースに保存しました');
                // 結果から削除
                setResults(prev => prev.filter(r => r !== data));
            }
            else {
                react_hot_toast_1.default.error('保存に失敗しました');
            }
        }
        catch (error) {
            react_hot_toast_1.default.error(`エラー: ${error}`);
    ...
    return (<div className="space-y-6">
      ...
      <div {...getRootProps()} className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors
        ...
        {isDragActive ? (<p className="text-lg">ファイルをドロップしてください</p>) : (<>
      ...
      {results.length > 0 && (<div className="space-y-4">
          ...
          {results.map((result, index) => (<div key={index} className="bg-white border rounded-lg p-4">
              <div className="flex justify-between items-start mb-3">
                <h4 className="font-medium">{result.file}</h4>
                <button onClick={() => saveToDatabase(result)} className="px-3 py-1 bg-green-600 text-white text-sm rounded hover:bg-green-700">
                  保存
                </button>
              </div>
              ...
              {result.items && result.items.length > 0 && (<div className="mt-3">
                  <p className="text-sm text-gray-500 mb-1">品目:</p>
                  <ul className="text-sm space-y-1">
                    {result.items.map((item, i) => (<li key={i} className="flex justify-between">
                        <span>{item.name}</span>
          ...
          <div className="flex justify-between items-center mt-6 pt-4 border-t">
            <button onClick={() => setResults([])} className="px-4 py-2 text-gray-600 hover:text-gray-900">
              結果をクリア
            </button>
            <a href="/documents" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
              書類一覧で確認
            </a>
          </div>
        </div>)}
