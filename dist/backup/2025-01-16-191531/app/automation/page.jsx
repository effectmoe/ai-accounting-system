...
const automation_scheduler_1 = require("@/services/automation-scheduler");
function AutomationPage() {
    const [tasks, setTasks] = (0, react_1.useState)([]);
    const [selectedTask, setSelectedTask] = (0, react_1.useState)(null);
    const [isRunning, setIsRunning] = (0, react_1.useState)({});
    ...
    const loadTasks = () => {
        const allTasks = automation_scheduler_1.scheduler.getTasks();
        setTasks(allTasks);
    };
    const handleToggleTask = (taskId, enabled) => {
        automation_scheduler_1.scheduler.toggleTask(taskId, enabled);
        loadTasks();
    };
    const handleRunTask = async (taskId) => {
        setIsRunning(prev => ({ ...prev, [taskId]: true }));
        try {
            await automation_scheduler_1.scheduler.runTask(taskId);
            alert('タスクを実行しました');
        }
        catch (error) {
    ...
    return (<div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
        ...
        <div className="grid gap-4">
          {tasks.map(task => (<div key={task.id} className="border rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-4">
                  <label className="flex items-center">
                    <input type="checkbox" checked={task.enabled} onChange={(e) => handleToggleTask(task.id, e.target.checked)} className="mr-2"/>
                    有効
                  </label>
                  <button onClick={() => handleRunTask(task.id)} disabled={isRunning[task.id]} className={`px-4 py-2 rounded ${isRunning[task.id]
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span className={`font-medium ${task.status === 'completed' ? 'text-green-600' :
              ...
              {task.lastRun && (<div className="mt-2 text-sm text-gray-500">
