...
function useColumnVisibility() {
    ...
    // 旧形式から新形式への移行
    const migrateOldFormat = (0, react_1.useCallback)((oldSettings) => {
    ...
    (0, react_1.useEffect)(() => {
        try {
            ...
            logger_1.logger.error('Failed to load column visibility settings:', error);
        }
    }, [migrateOldFormat]);
    // 設定をLocalStorageに保存
    const saveColumnConfigs = (0, react_1.useCallback)((configs) => {
        ...
        try {
            ...
            logger_1.logger.error('Failed to save column visibility settings:', error);
        }
    }, []);
    // 個別カラムの表示/非表示切り替え
...
function CustomersPageContent() {
    ...
    const getCityCandidates = (prefecture) => {
    ...
    const columnDefinitions = (0, react_1.useMemo)(() => [
        ...
        },
        {
            ...
            label: 'ウェブサイト',
            sortable: false,
            render: (customer) => customer.website ? (<a href={customer.website} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
          {customer.website}
        </a>) : '-',
        },
        {
        ...
        },
        {
            ...
            render: (customer) => {
                const contact = getPrimaryContact(customer);
                return contact.email ? (<a href={`mailto:${contact.email}`} className="text-blue-600 hover:underline">
            {contact.email}
          </a>) : '-';
            },
        },
        {
            ...
            render: (customer) => {
                const contact = getPrimaryContact(customer);
                return contact.phone ? (<a href={`tel:${contact.phone}`} className="text-blue-600 hover:underline">
            {contact.phone}
          </a>) : '-';
            },
        },
    ...
    const fetchCustomers = (0, react_1.useCallback)(async () => {
        try {
            ...
            logger_1.logger.error('Error loading customers:', error);
    ...
    const handleStatusUpdate = async (customerId, newStatus) => {
        ...
        try {
            ...
            // 成功時のトースト表示
            react_hot_toast_1.toast.success(data.message || `ステータスを${newStatus ? 'アクティブ' : '非アクティブ'}に更新しました`);
            ...
            logger_1.logger.error('ステータス更新エラー:', error);
            // エラー時は元の状態に戻す
    ...
    const deleteSelectedCustomers = async () => {
        ...
        try {
            const deletePromises = Array.from(selectedCustomers).map(customerId => fetch(`/api/customers/${customerId}`, { method: 'DELETE' }));
            ...
            if (failedDeletions > 0) {
                ...
                react_hot_toast_1.toast.success(`${selectedCustomers.size}件の顧客を削除しました`);
            }
            setSelectedCustomers(new Set());
            fetchCustomers();
        }
        catch (error) {
            logger_1.logger.error('Error deleting customers:', error);
    ...
    return (<div className="container mx-auto px-4 py-8">
      <div className="mb-6 flex justify-between items-center">
        ...
        <link_1.default href="/customers/new" className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 flex items-center gap-2">
      ...
      <div className="mb-6 bg-white rounded-lg shadow p-4">
        <form onSubmit={handleSearch} className="flex gap-2">
          <div className="flex-1 relative">
            ...
            <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="顧客コード、会社名、メールアドレス、部署名で検索..." className="w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"/>
          </div>
        ...
        {showFilters && (<div className="mt-4 p-4 border-t bg-gray-50 rounded-b-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select value={filters.prefecture || ''} onChange={(e) => {
                const value = e.target.value;
                if (value === '') {
                    const newFilters = { ...filters };
                  ...
                  {PREFECTURES.map((prefecture) => (<option key={prefecture} value={prefecture}>
                      {prefecture}
                    </option>))}
                </select>
              </div>
              ...
              <div>
                ...
                <select value={filters.city || ''} disabled={!filters.prefecture} onChange={(e) => {
                const value = e.target.value;
                if (value === '') {
                    const newFilters = { ...filters };
                    delete newFilters.city;
            ...
            }} className={`w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${!filters.prefecture ? 'bg-gray-100 cursor-not-allowed' : ''}`}>
                  <option value="">全ての市区町村</option>
                  {filters.prefecture && getCityCandidates(filters.prefecture).map((city) => (<option key={city} value={city}>
                      {city}
                    </option>))}
                </select>
                {!filters.prefecture && (<p className="text-xs text-gray-500 mt-1">
                    都道府県を選択してください
                  </p>)}
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input type="number" value={filters.paymentTermsMin || ''} onChange={(e) => {
                const value = e.target.value;
                if (value === '') {
                    const newFilters = { ...filters };
                  ...
                  <input type="number" value={filters.paymentTermsMax || ''} onChange={(e) => {
                const value = e.target.value;
                if (value === '') {
                    const newFilters = { ...filters };
        ...
        {getActiveFilterCount() > 0 && (<div className="mt-3 flex flex-wrap gap-2">
            ...
            {filters.prefecture && (<div className="inline-flex items-center gap-1 bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-sm">
                都道府県: {filters.prefecture}
                <button onClick={() => removeFilter('prefecture')} className="ml-1 hover:bg-blue-200 rounded-full p-0.5">
                  <lucide_react_1.X className="w-3 h-3"/>
                </button>
              </div>)}
            {filters.city && (<div className="inline-flex items-center gap-1 bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-sm">
                市区町村: {filters.city}
                <button onClick={() => removeFilter('city')} className="ml-1 hover:bg-blue-200 rounded-full p-0.5">
                  <lucide_react_1.X className="w-3 h-3"/>
                </button>
              </div>)}
            {(filters.paymentTermsMin !== undefined || filters.paymentTermsMax !== undefined) && (<div className="inline-flex items-center gap-1 bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-sm">
                支払いサイト: 
      ...
      <div className="bg-white rounded-lg shadow">
        {selectedCustomers.size > 0 && (<div className="flex gap-2 items-center p-4 bg-blue-50 border-b">
            <input type="checkbox" checked={selectedCustomers.size === customers.length} onChange={selectAllCustomers} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"/>
            <span className="text-sm">{selectedCustomers.size}件選択中</span>
            <button onClick={deleteSelectedCustomers} className="ml-auto text-red-600 hover:text-red-800 flex items-center gap-1 text-sm">
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="p-4 text-left">
                  <input type="checkbox" checked={selectedCustomers.size === customers.length && customers.length > 0} onChange={selectAllCustomers} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"/>
                </th>
            ...
            <tbody>
              {customers.length === 0 ? (<tr>
                ...
                </tr>) : (customers.map((customer) => (<tr key={customer.id || customer._id} className="border-b hover:bg-gray-50">
                    <td className="p-4">
                      <input type="checkbox" checked={selectedCustomers.has(customer.id || customer._id?.toString() || '')} onChange={() => toggleCustomerSelection(customer.id || customer._id?.toString() || '')} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"/>
                    </td>
                    {visibleColumns.map((column) => (<td key={column.key} className={`p-4 text-sm ${column.key === 'companyName' ? 'font-medium' : 'text-gray-600'}`}>
            ...
            }} disabled={updatingStatus.has(customer.id || customer._id?.toString() || '')} className={`inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-full transition-all duration-200 ${customer.isActive !== false
                ? 'bg-green-100 text-green-800 hover:bg-green-200'
                : 'bg-gray-100 text-gray-800 hover:bg-gray-200'} ${updatingStatus.has(customer.id || customer._id?.toString() || '')
                ? 'opacity-50 cursor-not-allowed'
                : 'cursor-pointer hover:shadow-sm'}`} title="クリックでステータスを切り替え">
                        {updatingStatus.has(customer.id || customer._id?.toString() || '') && (<div className="animate-spin w-3 h-3 border border-current border-t-transparent rounded-full"/>)}
                        {customer.isActive !== false ? 'アクティブ' : '非アクティブ'}
                      </button>
                    </td>
                    <td className="p-4">
                      <div className="flex gap-2">
                        <link_1.default href={`/customers/${customer.id || customer._id}/edit`} className="text-blue-600 hover:text-blue-800 p-1" title="編集">
                          <lucide_react_1.Edit className="w-4 h-4"/>
                        </link_1.default>
                        <button onClick={() => handleDeleteCustomer(customer.id || customer._id?.toString() || '')} className="text-red-600 hover:text-red-800 p-1" title="削除">
                          <lucide_react_1.Trash2 className="w-4 h-4"/>
                        </button>
                      </div>
                    </td>
        ...
        {totalPages > 1 && (<div className="flex items-center justify-between p-4 border-t">
            ...
            <div className="flex gap-2">
              ...
              <div className="flex gap-1">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (<button key={page} onClick={() => setCurrentPage(page)} className={`px-3 py-1 rounded ${currentPage === page
