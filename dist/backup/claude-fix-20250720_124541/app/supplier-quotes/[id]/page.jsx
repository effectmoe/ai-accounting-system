      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="flex items-start gap-2">
                  <lucide_react_1.Building className="h-5 w-5 text-gray-400 mt-0.5"/>
                  <div className="flex flex-col space-y-1">
                    <span className="text-gray-900 font-medium">
                      {quote.supplier?.companyName || quote.vendor?.name || quote.vendorName || '未設定'}
                    </span>
                    {(quote.supplier?.address || quote.vendorAddress) && (<span className="text-sm text-gray-600">
                        <span className="font-medium">住所:</span> {quote.supplier?.address || quote.vendorAddress}
                      </span>)}
                    {(quote.supplier?.contactPhone || quote.vendorPhone) && (<span className="text-sm text-gray-600">
                    ...
                    {(quote.supplier?.contactEmail || quote.vendorEmail) && (<span className="text-sm text-gray-600">
            ...
            {/* 赤枠の4項目 */}
            {(quote.subject || quote.deliveryLocation || quote.paymentTerms || quote.quotationValidity) && (<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 pt-4 border-t border-gray-200">
                {quote.subject && (<div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      件名
                    </label>
                    <div className="text-gray-900">{quote.subject}</div>
                  </div>)}
                {quote.deliveryLocation && (<div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      納入場所
                    </label>
                    <div className="text-gray-900">{quote.deliveryLocation}</div>
                  </div>)}
                {quote.paymentTerms && (<div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      お支払条件
                    </label>
                    <div className="text-gray-900">{quote.paymentTerms}</div>
                  </div>)}
                {quote.quotationValidity && (<div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      見積有効期限
                    </label>
          ...
          {/* OCR元ファイル */}
          {(quote.ocrResultId || quote.fileId) && (<div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  ...
                  <div className="flex gap-2">
                    {quote.fileId && (<>
                        <a href={`/api/documents/${quote.fileId}/download`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors cursor-pointer">
                        ...
                        <a href={`/api/documents/${quote.fileId}/download`} download className="inline-flex items-center gap-2 px-4 py-2 bg-gray-50 text-gray-700 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer">
                          <lucide_react_1.Download size={16}/>
                          ダウンロード
                        </a>
                      </>)}
                    {quote.ocrResultId && (<a href={`/api/ocr-results/${quote.ocrResultId}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-gray-50 text-gray-700 rounded-lg hover:bg-gray-100 transition-colors">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {quote.items.map((item, index) => (<tr key={index}>
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        {item.remarks && (<div className="text-sm text-gray-600 mt-1">
          ...
          {(() => {
            const itemRemarks = quote.items
                ...
                .join('\n');
            if (itemRemarks) {
                return (<div className="bg-yellow-50 rounded-lg shadow-sm border border-yellow-200 p-6">
