        ...
        const fetchSupplier = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    logger_1.logger.debug('Fetched supplier data:', data);
                    logger_1.logger.debug('Bank transfer info:', data.bankTransferInfo);
                    logger_1.logger.debug('Has bank transfer info:', !!data.bankTransferInfo);
                    logger_1.logger.debug('Bank transfer info length:', data.bankTransferInfo?.length);
                    setSupplier(data);
                }
                else {
                    logger_1.logger.error('Failed to fetch supplier');
                }
            }
            catch (error) {
    ...
    return (<div className="container mx-auto py-6 space-y-6">
      ...
      <tabs_1.Tabs defaultValue="info" className="space-y-4">
        ...
        <tabs_1.TabsContent value="info" className="space-y-4">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                <div className="space-y-2">
                  ...
                  {(supplier.prefecture || supplier.city || supplier.address1 || supplier.address2 || supplier.address) ? (<div>
                      {/* addressフィールドが存在する場合（新しいフォーマット） */}
                      {supplier.address && (<p>
                          {supplier.address}
                        </p>)}
                      {/* 都道府県と市区町村が存在する場合（旧フォーマット） */}
                      {!(supplier.address) && (supplier.prefecture || supplier.city) && (<p>
                          {supplier.prefecture}
                          {supplier.city}
                        </p>)}
                      {/* 住所1が存在する場合（旧フォーマット） */}
                      {!(supplier.address) && supplier.address1 && (<p>
                          {supplier.address1}
                        </p>)}
                      {/* 住所2が存在する場合 */}
                      {supplier.address2 && (<p>
                    ...
                    </div>) : (<p className="text-muted-foreground">住所情報が登録されていません</p>)}
                </div>
              </card_1.CardContent>
            </card_1.Card>
            ...
            <card_1.Card>
              ...
              <card_1.CardContent className="space-y-4">
                {supplier.phone || (supplier.fax && supplier.fax.trim() !== '') || supplier.email || supplier.website ? (<>
                    ...
                    {supplier.fax && supplier.fax.trim() !== '' && (<div className="flex items-center gap-2">
          ...
          {/* 振込先情報 */}
          {(() => {
            logger_1.logger.debug('Bank transfer info check:', {
          ...
          {supplier.bankTransferInfo && Array.isArray(supplier.bankTransferInfo) && supplier.bankTransferInfo.length > 0 && (<card_1.Card>
              <card_1.CardHeader>
              ...
              <card_1.CardContent>
                <div className="space-y-4">
                  {supplier.bankTransferInfo.map((bank, index) => (<div key={index} className="border rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {bank.additionalInfo && (<div className="md:col-span-2">
