...
async function GET() {
    try {
        ...
        const ocrResults = await mongodb_client_1.db.find('ocr_results', {
            $or: [
        ...
        // 統計情報
        const ocrResultsCount = await mongodb_client_1.db.count('ocr_results', {
            $or: [
                { company_id: companyId },
                { companyId: companyId }
            ]
        });
        const documentsCount = await mongodb_client_1.db.count('documents', {
            companyId: companyId,
            $or: [
                { ocrStatus: { $exists: true } },
                { documentType: 'receipt' },
        ...
        logger_1.logger.error('Debug OCR data error:', error);
        return server_1.NextResponse.json({
            success: false,
