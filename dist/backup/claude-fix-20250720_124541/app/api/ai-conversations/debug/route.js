...
async function GET(request) {
    try {
        ...
        logger_1.logger.debug('[AI Conversations Debug] Parameters:', { limit, conversationId, invoiceId });
        // 特定の会話を検索
        if (conversationId || invoiceId) {
            ...
            logger_1.logger.debug('[AI Conversations Debug] Trying queries:', queries);
            for (const query of queries) {
                const result = await conversationsCollection.findOne(query);
                if (result) {
                    logger_1.logger.debug('[AI Conversations Debug] Found with query:', query);
            ...
            if (invoiceId) {
                ...
                if (regexResult) {
                    logger_1.logger.debug('[AI Conversations Debug] Found with regex search');
                    return server_1.NextResponse.json({
                        success: true,
        ...
        logger_1.logger.error('デバッグエンドポイントエラー:', error);
