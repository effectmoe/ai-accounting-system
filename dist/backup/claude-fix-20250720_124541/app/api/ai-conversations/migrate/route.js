...
async function POST(request) {
    try {
        ...
        logger_1.logger.debug('[Migration] Found numeric conversationIds:', numericConversations.length);
        let migratedCount = 0;
        const errors = [];
        for (const conversation of numericConversations) {
            try {
                ...
                migratedCount++;
                logger_1.logger.debug(`[Migration] Migrated: ${conversation.conversationId} -> ${normalizedId}`);
            }
            catch (error) {
                errors.push({
...
async function GET(request) {
    try {
        ...
        logger_1.logger.error('Migration status error:', error);
