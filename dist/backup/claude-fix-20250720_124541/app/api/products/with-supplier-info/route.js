    ...
    try {
        ...
        if (productId) {
            ...
            const supplierInfo = await db.collection('productSupplierInfo')
                .aggregate([
                { $match: { productId: new mongodb_1.ObjectId(productId) } },
                {
                    $lookup: {
                        from: 'suppliers',
        ...
        logger_1.logger.error('Error fetching products with supplier info:', error);
        return server_1.NextResponse.json({ error: 'Failed to fetch products with supplier info' }, { status: 500 });
    }
}
// POST: 商品仕入情報の追加/更新
async function POST(request) {
    try {
        ...
        if (!data.productId || !data.supplierId || !data.purchasePrice) {
            return server_1.NextResponse.json({ error: 'Product ID, Supplier ID, and purchase price are required' }, { status: 400 });
        }
        const client = await (0, mongodb_client_1.getMongoClient)();
        const db = client.db(DB_NAME);
        // 既存の仕入情報をチェック
        const existing = await db.collection('productSupplierInfo').findOne({
            ...
            supplierId: new mongodb_1.ObjectId(data.supplierId)
        });
        const supplierInfo = {
            productId: new mongodb_1.ObjectId(data.productId),
            supplierId: new mongodb_1.ObjectId(data.supplierId),
        ...
        if (existing) {
            // 更新
            await db.collection('productSupplierInfo').updateOne({ _id: existing._id }, { $set: supplierInfo });
        }
        else {
            // 新規作成
        ...
        if (data.isPreferred) {
            await db.collection('productSupplierInfo').updateMany({
                productId: new mongodb_1.ObjectId(data.productId),
                supplierId: { $ne: new mongodb_1.ObjectId(data.supplierId) }
            }, { $set: { isPreferred: false } });
        }
        return server_1.NextResponse.json({ success: true });
    }
    catch (error) {
        logger_1.logger.error('Error updating product supplier info:', error);
        return server_1.NextResponse.json({ error: 'Failed to update product supplier info' }, { status: 500 });
    }
}
// DELETE: 商品仕入情報の削除
async function DELETE(request) {
    try {
        ...
        if (!productId || !supplierId) {
            return server_1.NextResponse.json({ error: 'Product ID and Supplier ID are required' }, { status: 400 });
        }
        const client = await (0, mongodb_client_1.getMongoClient)();
        const db = client.db(DB_NAME);
        ...
        if (result.deletedCount === 0) {
            return server_1.NextResponse.json({ error: 'Product supplier info not found' }, { status: 404 });
        }
        return server_1.NextResponse.json({ success: true });
    }
    catch (error) {
        logger_1.logger.error('Error deleting product supplier info:', error);
        return server_1.NextResponse.json({ error: 'Failed to delete product supplier info' }, { status: 500 });
    }
}
