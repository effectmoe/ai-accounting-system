...
const payment_terms_1 = require("@/utils/payment-terms");
const logger_1 = require("@/lib/logger");
// AI クライアントの初期化
let openaiClient = null;
// DeepSeek APIを直接呼び出す関数
async function callDeepSeekAPI(messages, temperature = 0.7, maxTokens = 500) {
    const apiKey = process.env.DEEPSEEK_API_KEY;
    if (!apiKey) {
        logger_1.logger.error('[DeepSeek] API key is not configured');
        throw new Error('DeepSeek API key is not configured');
    }
    logger_1.logger.debug('[DeepSeek] Calling API with fetch');
    logger_1.logger.debug('[DeepSeek] API key length:', apiKey.length);
    logger_1.logger.debug('[DeepSeek] API key prefix:', apiKey.substring(0, 10) + '...');
    // タイムアウトを設定（8秒）
    ...
    try {
        ...
        if (!response.ok) {
            ...
            logger_1.logger.error('[DeepSeek] API error:', response.status);
            throw new Error(`DeepSeek API error: ${response.status}`);
        }
        const data = await response.json();
        logger_1.logger.debug('[DeepSeek] API response received');
        return data;
    }
    catch (error) {
        clearTimeout(timeoutId);
        if (error instanceof Error && error.name === 'AbortError') {
            logger_1.logger.error('[DeepSeek] Request timeout');
            throw new Error('DeepSeek API request timeout');
        }
        logger_1.logger.error('[DeepSeek] Fetch error:', error);
        throw error;
    }
}
...
// レスポンススキーマ
const InvoiceDataSchema = zod_1.z.object({
    ...
    items: zod_1.z.array(zod_1.z.object({
        ...
        taxRate: zod_1.z.number().default(0.1),
        taxAmount: zod_1.z.number(),
    })),
    invoiceDate: zod_1.z.string().optional(),
    dueDate: zod_1.z.string().optional(),
    notes: zod_1.z.string().optional(),
    paymentMethod: zod_1.z.enum(['bank_transfer', 'credit_card', 'cash', 'other']).default('bank_transfer'),
});
async function POST(request) {
    ...
    try {
        logger_1.logger.debug('[API Start] analyze-chat endpoint called at', new Date().toISOString());
        const body = await request.json();
        logger_1.logger.debug('Analyze chat request body:', body);
        const { conversation, conversationHistory, sessionId, currentInvoiceData, mode = 'create', initialInvoiceData } = body;
        if (!conversation || typeof conversation !== 'string') {
            logger_1.logger.debug('Invalid conversation data:', conversation);
            return server_1.NextResponse.json({ error: 'Conversation text is required' }, { status: 400 });
        }
        logger_1.logger.debug('Conversation to analyze:', conversation);
        logger_1.logger.debug('Session ID:', sessionId);
        logger_1.logger.debug('Mode:', mode);
        logger_1.logger.debug('[DEBUG] Current invoice data received:', JSON.stringify(currentInvoiceData, null, 2));
        // 重要：currentInvoiceDataのitemsを確認
        if (currentInvoiceData && currentInvoiceData.items) {
            logger_1.logger.debug('[DEBUG] Current items count:', currentInvoiceData.items.length);
            currentInvoiceData.items.forEach((item, index) => {
                logger_1.logger.debug(`[DEBUG] Item ${index + 1}:`, {
        ...
        logger_1.logger.debug('API Keys status:', {
        ...
        if (openaiApiKey) {
            try {
                ...
                logger_1.logger.debug('OpenAI API key is invalid or not configured properly');
                hasOpenAI = false;
            }
        }
        ...
        if (hasDeepSeek || hasOpenAI) {
            ...
            logger_1.logger.debug(`Using ${usingDeepSeek ? 'DeepSeek' : 'OpenAI'} API for conversation analysis`);
            // 会社情報から支払い条件を取得
            let paymentTerms = '';
            ...
            const invoiceDate = (0, date_fns_1.format)(today, 'yyyy-MM-dd');
            // 支払い条件に基づいて支払期限を計算
            const calculatedDueDate = (0, payment_terms_1.calculateDueDate)(today, paymentTerms);
            const dueDate = (0, date_fns_1.format)(calculatedDueDate, 'yyyy-MM-dd');
            // 自然な対話のためのプロンプト（簡潔版）
            let systemPrompt = '';
            if (mode === 'edit' && currentInvoiceData) {
                // 編集モードの場合、既存データを詳細に含める
                const items = currentInvoiceData.items || [];
                const currentDetails = items.map((item, index) => `${index + 1}. ${item.description || item.itemName}: ¥${((item.amount || 0) + (item.taxAmount || 0)).toLocaleString()}`).join('\n');
...
今日: ${invoiceDate}、支払期限: ${dueDate}`;
            ...
            try {
                logger_1.logger.debug('[AI] Starting AI processing', { usingDeepSeek });
                ...
                logger_1.logger.debug('Sending to AI with messages:', messages.length);
                try {
                    ...
                    logger_1.logger.error('[AI] API call failed:', {
                        error: apiError instanceof Error ? apiError.message : 'Unknown error',
                        stack: apiError instanceof Error ? apiError.stack : undefined,
                        model: usingDeepSeek ? 'deepseek-chat' : (process.env.AZURE_OPENAI_DEPLOYMENT_NAME || 'gpt-4'),
                        usingDeepSeek,
                        responseStatus: apiError instanceof Error && 'response' in apiError ? apiError.response?.status : undefined,
                        responseData: apiError instanceof Error && 'response' in apiError ? apiError.response?.data : undefined
                    });
                    // DeepSeekが失敗した場合、OpenAIを試す
                    if (usingDeepSeek && hasOpenAI) {
                        logger_1.logger.debug('[AI] DeepSeek failed, trying OpenAI as fallback');
                        try {
                            ...
                            logger_1.logger.debug('[AI] OpenAI fallback successful');
                        }
                        catch (openaiError) {
                            logger_1.logger.error('[AI] OpenAI fallback also failed:', openaiError);
                            // 両方失敗した場合はパターンマッチングに進む
                            logger_1.logger.debug('Both AI services failed, falling back to pattern matching logic');
                        }
                    }
                    else {
                        // APIエラーの場合はフォールバックロジックに進む
                        logger_1.logger.debug('Falling back to pattern matching logic due to API error');
                    }
                }
                // AIレスポンスからデータを抽出
                ...
                if (aiResponse) {
                    logger_1.logger.debug('[AI] Processing AI response for data extraction');
                    logger_1.logger.debug('[AI] AI Response:', aiResponse);
                    // シンプルなキーワードベースの抽出
                    ...
                    if (mode === 'edit') {
                        ...
                        logger_1.logger.debug('[AI] Edit mode - Original customer name:', originalCustomerName);
                        ...
                        for (const pattern of customerNamePatterns) {
                            ...
                            if (match) {
                                ...
                                if (extractedName && extractedName !== 'Unknown Customer' && extractedName !== '未設定') {
                                    matchFound = true;
                                    logger_1.logger.debug('[AI] Customer name change detected with pattern:', pattern.source);
                                    logger_1.logger.debug('[AI] Extracted customer name:', extractedName);
                                    break;
                                }
                            }
                        }
                        if (matchFound) {
                            ...
                            logger_1.logger.debug('[AI] Customer name explicitly changed to:', updatedData.customerName);
                        }
                        else {
                            ...
                            for (const pattern of userConversationPatterns) {
                                ...
                                if (match && match[1]) {
                                    ...
                                    logger_1.logger.debug('[AI] User requested customer name from conversation:', userRequestedName);
                                    break;
                                }
                            }
                            if (userRequestedName && userRequestedName !== originalCustomerName) {
                                updatedData.customerName = userRequestedName;
                                logger_1.logger.debug('[AI] Customer name changed based on user request:', updatedData.customerName);
                            }
                            else {
                                // 顧客名の変更指示がない場合は元の顧客名を維持
                                updatedData.customerName = originalCustomerName;
                                logger_1.logger.debug('[AI] No customer name change detected, keeping original:', originalCustomerName);
                            }
                        }
                    }
                    ...
                    if (mode === 'create' && !updatedData.customerName && conversation) {
                        ...
                        if (customerMatch) {
                            ...
                            logger_1.logger.debug('[AI] Extracted customer name from conversation:', updatedData.customerName);
                        }
                    }
                    // まず、ユーザーの入力メッセージから金額変更の指示を確認
                    logger_1.logger.debug('[AI] Checking user conversation for amount:', conversation);
                    ...
                    if (userAmountMatch && mode === 'edit') {
                        ...
                        logger_1.logger.debug(`[AI] Found amount in user input: ${amount}円 (tax excluded)`);
                        logger_1.logger.debug('[AI] Current invoice items before update:', JSON.stringify(currentInvoiceData?.items, null, 2));
                        // 既存のアイテムを更新
                        if (currentInvoiceData?.items && currentInvoiceData.items.length > 0) {
                            newItems = currentInvoiceData.items.map((item, index) => {
                                if (index === 0) { // 最初のアイテムの金額を更新
                                    logger_1.logger.debug('[AI] Updating item:', {
                            ...
                            foundItemList = true;
                            logger_1.logger.debug('[AI] Updated items based on user input:', JSON.stringify(newItems, null, 2));
                        }
                    }
                    else {
                        logger_1.logger.debug('[AI] No amount found in user input or not in edit mode. Mode:', mode);
                    }
                    ...
                    for (const line of lines) {
                        ...
                        if (itemMatch) {
                            ...
                            logger_1.logger.debug(`[AI] Found item ${itemNumber}: ${description} = ${amount}円`);
                            newItems.push({
                    ...
                    for (const line of lines) {
                        ...
                        if (taxExcludedMatch) {
                            ...
                            const amount = parseInt(amountStr);
                            logger_1.logger.debug(`[AI] Found tax-excluded amount update request: ${amount}円`);
                        ...
                        if (taxSeparateMatch && !amountUpdateFound) {
                            ...
                            const amount = parseInt(amountStr);
                            logger_1.logger.debug(`[AI] Found tax-separate amount pattern: ${amount}円 (tax excluded)`);
                            amountUpdateFound = true;
                            // 既存のアイテムまたは新規アイテムを更新
                            if (currentInvoiceData?.items && currentInvoiceData.items.length > 0) {
                                newItems = currentInvoiceData.items.map((item, index) => {
                        ...
                        if (changeMatch) {
                            ...
                            logger_1.logger.debug(`[AI] Found amount change: ${amount}円 (tax excluded)`);
                            amountUpdateFound = true;
                            // 既存のアイテムまたは新規アイテムを更新
                            if (currentInvoiceData?.items && currentInvoiceData.items.length > 0) {
                                newItems = currentInvoiceData.items.map((item, index) => {
                        ...
                        if (itemUpdateMatch) {
                            ...
                            logger_1.logger.debug(`[AI] Found item with tax info: ${description} = ${taxExcluded}円 (tax excluded)`);
                            // この項目で既存のアイテムを更新
                            newItems = [{
                                    description: description,
                                    quantity: 1,
                    ...
                    if (amountUpdateFound) {
                        logger_1.logger.debug('[AI] Amount update found, newItems:', JSON.stringify(newItems, null, 2));
                    }
                    // 合計金額の確認（税込み）
                    ...
                    for (const line of lines) {
                        ...
                        if (totalMatch) {
                            ...
                            logger_1.logger.debug('[AI] Found total with tax:', totalWithTax);
                            break;
                        }
                    }
                    ...
                    if (foundItemList && newItems.length > 0) {
                        ...
                        logger_1.logger.debug('[AI] Updated items from AI response:', JSON.stringify(newItems, null, 2));
                    }
                    // 編集モードの場合、日付も抽出
                    if (mode === 'edit' && aiResponse) {
                        ...
                        if (invoiceDateMatch) {
                            ...
                            logger_1.logger.debug('[AI] Extracted invoice date from AI response:', updatedData.invoiceDate);
                        }
                        // 支払期限の抽出（例：「支払期限: 2025-08-31」）
                        const dueDateMatch = aiResponse.match(/支払期限[：:]\s*(\d{4}-\d{2}-\d{2})/);
                        if (dueDateMatch) {
                            updatedData.dueDate = dueDateMatch[1];
                            logger_1.logger.debug('[AI] Extracted due date from AI response:', updatedData.dueDate);
                        }
                    }
                    // extractedDataに更新されたデータを設定
                    extractedData = updatedData;
                    logger_1.logger.debug('[AI] Final extracted data:', JSON.stringify(extractedData, null, 2));
                }
                // フォールバック処理 - AIが番号付き項目を出力しなかった場合
                if (!extractedData || !extractedData.items || extractedData.items.length === 0) {
                    logger_1.logger.debug('[AI] No items found in AI response, keeping current data');
                ...
                try {
                    ...
                    logger_1.logger.error('Error fetching bank account:', error);
                    // エラーが発生しても処理は続行
                }
                // 備考に支払い条件と振込先情報を追加
                ...
                // デバッグログ
                logger_1.logger.debug('[API/AI] Final data before response:', {
                ...
                // デバッグログを追加
                logger_1.logger.debug('[API/AI] Final data for response generation:', {
                    mode,
                ...
                logger_1.logger.debug('[API/AI] Response data customer name:', responseData.customerName);
                const response = {
                    success: true,
                ...
                logger_1.logger.debug('[API] Final response being sent:', JSON.stringify(response, null, 2));
                logger_1.logger.debug('[API] Response items count:', response.data.items?.length || 0);
                logger_1.logger.debug('[API] Response subtotal:', response.data.subtotal);
                logger_1.logger.debug('[API] Response total:', response.data.totalAmount);
                // 各アイテムの詳細をログ
                if (response.data.items && response.data.items.length > 0) {
                    logger_1.logger.debug('[API] Response items detail:');
                    response.data.items.forEach((item, index) => {
                        logger_1.logger.debug(`[API] Item ${index}:`, {
                            description: item.description,
                            quantity: item.quantity,
                ...
                logger_1.logger.error(`${usingDeepSeek ? 'DeepSeek' : 'OpenAI'} API error:`, error);
                logger_1.logger.error('[DEBUG] Error details:', {
                ...
                logger_1.logger.debug(`${usingDeepSeek ? 'DeepSeek' : 'OpenAI'} API failed, falling back to placeholder implementation`);
                // フォールバック：プレースホルダー実装へ続く
            }
        }
        // プレースホルダー実装（AIが使えない場合の最小限のフォールバック）
        logger_1.logger.debug('[DEBUG] Using minimal placeholder implementation - AI models not available or failed');
        logger_1.logger.debug('[DEBUG] Environment check:', {
        ...
        logger_1.logger.debug('[Placeholder] Minimal extraction:', {
            conversation,
            amount,
            customerName
        });
        ...
        if (customerName && customerName !== '未設定顧客') {
            try {
                ...
                // 顧客名で部分一致検索
                const matchedCustomers = searchResult.customers.filter(c => c.companyName.includes(customerName) ||
                ...
                logger_1.logger.error('Customer search error:', err);
            }
        }
        // 質問タイプを分析する関数
        const analyzeQuestionType = (message) => {
            // 複数質問の検出
            ...
            // 情報要求パターン（拡充版）
            const isInfoRequest = (message.includes('お知らせください') ||
            ...
            // Yes/No質問パターン（厳密化）
            const isYesNoQuestion = (!hasMultipleQuestions && !isInfoRequest && (message.includes('よろしいですか') ||
            ...
            // 選択質問パターン
            const isChoiceQuestion = (!hasMultipleQuestions && !isInfoRequest && (message.includes('どの') ||
            ...
            const shouldShowQuickReplies = (!hasMultipleQuestions &&
                !isInfoRequest &&
        ...
        logger_1.logger.debug('[API] Response data calculation:', {
        ...
        logger_1.logger.error('Error analyzing conversation:', error);
        // タイムアウトエラーを明確に返す
