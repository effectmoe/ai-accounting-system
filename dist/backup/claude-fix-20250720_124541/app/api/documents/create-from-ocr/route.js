...
async function POST(request) {
    try {
        ...
        logger_1.logger.debug('[Create Document] Received request body:', body);
        // AI駆動のOCRオーケストレーターを使用した構造化データの処理
        if (body.aiStructuredData) {
            logger_1.logger.debug('[Create Document] Using AI-driven structured data');
            return await handleAIStructuredData(body.aiStructuredData, body.companyId);
        }
        // 従来のOCRデータの処理（後方互換性のため）
        logger_1.logger.debug('[Create Document] Using legacy OCR data format');
        return await handleLegacyOCRData(body);
    }
    catch (error) {
        logger_1.logger.error('Create document from OCR error:', error);
        const errorMessage = error instanceof Error ? error.message : '文書の作成に失敗しました';
        return server_1.NextResponse.json({ error: errorMessage, details: error instanceof Error ? error.stack : undefined }, { status: 500 });
    }
}
/**
 * AI駆動のOCRオーケストレーターによる構造化データを処理
 */
async function handleAIStructuredData(aiData, companyId) {
    logger_1.logger.debug('[Create Document] Processing AI structured data:', {
    ...
    // 文書を保存
    const savedDocument = await document_service_1.DocumentService.saveDocument(documentData, companyId || '11111111-1111-1111-1111-111111111111');
 ...
 */
async function handleLegacyOCRData(body) {
    const { ocrResultId, vendor_name, receipt_date, subtotal_amount = 0, tax_amount = 0, total_amount = 0, payment_amount = 0, change_amount = 0, receipt_number = '', store_name = '', store_phone = '', company_name = '', notes = '', file_name } = body;
    ...
    // 文書を保存
    const savedDocument = await document_service_1.DocumentService.saveDocument(documentData, companyId);
    return server_1.NextResponse.json({
        id: savedDocument.id,
