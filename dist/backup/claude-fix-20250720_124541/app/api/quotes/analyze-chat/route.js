...
async function callDeepSeekAPI(messages, temperature = 0.7, maxTokens = 500) {
    ...
    if (!apiKey.startsWith('sk-') || apiKey.length < 20) {
        logger_1.logger.error('[DeepSeek] API key format appears to be invalid');
        logger_1.logger.error('[DeepSeek] API key should start with "sk-" and be at least 20 characters');
    }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            logger_1.logger.error('[DeepSeek] Error response:', errorText);
            logger_1.logger.error('[DeepSeek] Request details:', {
                model: 'deepseek-chat',
...
// レスポンススキーマ（見積書用）
const QuoteDataSchema = zod_1.z.object({
    ...
    quoteDate: zod_1.z.string().optional(), // 見積書の発行日
    validityDate: zod_1.z.string().optional(), // 見積書の有効期限
    notes: zod_1.z.string().optional(),
    paymentMethod: zod_1.z.string().optional(),
    customerId: zod_1.z.string().optional()
});
async function POST(request) {
    logger_1.logger.debug('[API] POST /api/quotes/analyze-chat - Start');
    let body;
    try {
        body = await request.json();
        logger_1.logger.debug('[API] Request body:', JSON.stringify(body, null, 2));
        const { conversation, conversationHistory, currentInvoiceData, sessionId, mode, initialInvoiceData } = body;
        if (!conversation) {
            return server_1.NextResponse.json({ error: 'Conversation is required' }, { status: 400 });
        }
        // 顧客データの取得
        ...
        logger_1.logger.debug('[API] Number of customers:', customersResult.customers.length);
        // 会社情報の取得
        ...
        const defaultQuoteDate = (0, date_fns_1.format)(today, 'yyyy-MM-dd');
        // デフォルトの有効期限（30日後）
        const defaultValidityDate = new Date();
        defaultValidityDate.setDate(defaultValidityDate.getDate() + 30);
        const formattedValidityDate = (0, date_fns_1.format)(defaultValidityDate, 'yyyy-MM-dd');
        // 顧客リストをテキスト形式で準備
        const customerList = customersResult.customers.map(customer => `ID: ${customer._id}, 会社名: ${customer.companyName || customer.name || '名前未設定'}, 電話: ${customer.phone || '未設定'}, メール: ${customer.email || '未設定'}`).join('\n');
        // システムプロンプト（見積書用）
...
金額や項目が含まれる場合は、番号付きリストで明確に表示してください。`;
        try {
            logger_1.logger.debug('[API] Calling DeepSeek API for quote analysis');
            const messages = [
                { role: 'system', content: systemPrompt },
                { role: 'user', content: conversation }
            ];
            logger_1.logger.debug('[API] Messages to send:', JSON.stringify(messages, null, 2));
            const completion = await callDeepSeekAPI(messages, 0.7, 1000);
            if (!completion.choices || completion.choices.length === 0) {
                throw new Error('No response from AI service');
            }
            const aiResponse = completion.choices[0].message.content;
            logger_1.logger.debug('[API] AI Response:', aiResponse);
            ...
            if (aiResponse) {
                logger_1.logger.debug('[API] Processing AI response for data extraction');
                // シンプルなキーワードベースの抽出
                ...
                if (mode === 'create' && !updatedData.customerName && conversation) {
                    ...
                    if (customerMatch) {
                        ...
                        logger_1.logger.debug('[API] Extracted customer name from conversation:', updatedData.customerName);
                    }
                }
                ...
                for (const line of lines) {
                    ...
                    if (itemMatch) {
                        ...
                        logger_1.logger.debug(`[API] Found item ${itemNumber}: ${description} = ${amount}円`);
                        newItems.push({
                ...
                if (foundItemList && newItems.length > 0) {
                    ...
                    logger_1.logger.debug('[API] Updated items from AI response:', JSON.stringify(newItems, null, 2));
                }
            ...
            if (!extractedData || !extractedData.items || extractedData.items.length === 0) {
                logger_1.logger.debug('[API] No items found in AI response, keeping current data');
            ...
            logger_1.logger.error('[API] AI processing error:', aiError);
            // フォールバック応答
            return server_1.NextResponse.json({
                success: false,
        ...
        logger_1.logger.error('[API] Error in analyze-chat:', error);
        logger_1.logger.error('[API] Error stack:', error instanceof Error ? error.stack : 'No stack trace');
        logger_1.logger.error('[API] Request data:', {
