...
function DocumentsContent() {
    ...
    const [activeTab, setActiveTab] = (0, react_1.useState)(searchParams.get('tab') || 'ocr');
    ...
    return (<div className="flex h-screen bg-gray-100">
      <div className="flex-1 flex flex-col">
        ...
        // OCR結果（カード形式）
        ocrResults.length === 0 ? (<div className="p-8 text-center">
                    <lucide_react_1.FileText className="mx-auto h-12 w-12 text-gray-400"/>
                    <p className="mt-2 text-gray-600">OCR処理済みの書類がありません</p>
                  </div>) : (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-4 md:p-6">
                    {ocrResults.map((result) => (<div key={result.id} className="bg-white border border-gray-200 rounded-lg hover:shadow-lg transition-all duration-200">
                        <div className="p-4">
                          {/* ヘッダー */}
                          <div className="mb-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-4 md:p-6">
                    {documents.map((doc) => (<div key={doc.id} className="bg-white border border-gray-200 rounded-lg hover:shadow-lg transition-all duration-200">
                        <div className="p-4">
                          {/* ヘッダー */}
                          <div className="flex justify-between items-start mb-3">
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-gray-500 mt-0.5">
                                {documentTypeLabels[(doc.documentType || doc.document_type || doc.type)] || '領収書'}
                              </p>
                            </div>
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[doc.status] || 'bg-gray-100 text-gray-800'}`}>
                              {statusLabels[doc.status] || doc.status || '下書き'}
                            </span>
                          </div>
