...
const ocr_processor_1 = require("@/lib/ocr-processor");
const DB_NAME = process.env.MONGODB_DB_NAME || 'accounting-app';
// 仕入見積書用のOCR処理
async function processSupplierQuoteOCR(ocrResult) {
...
async function POST(request) {
    try {
        ...
        if (!file) {
            return server_1.NextResponse.json({ error: 'File is required' }, { status: 400 });
        }
        ...
        // OCR処理を実行
        const ocrResult = await (0, ocr_processor_1.processOCR)(file);
        // OCR結果を保存
        const client = await (0, mongodb_client_1.getMongoClient)();
        const db = client.db(DB_NAME);
        const savedOcrResult = await db.collection('ocrResults').insertOne({
            ...ocrResult,
            createdAt: new Date(),
            updatedAt: new Date()
        });
        ...
        const supplierQuote = {
            ...quoteData,
            supplierId: new mongodb_1.ObjectId(finalSupplierId),
            dealId: dealId ? new mongodb_1.ObjectId(dealId) : undefined,
            attachments: [file.name], // ファイル名を保存
            createdAt: new Date(),
            updatedAt: new Date()
        };
        // 仕入見積書を保存
        const result = await db.collection('supplierQuotes').insertOne(supplierQuote);
        // 保存された仕入見積書を返す
        const savedQuote = {
            ...supplierQuote,
        ...
        const supplier = await db.collection('suppliers').findOne({
            _id: new mongodb_1.ObjectId(finalSupplierId)
        });
        if (supplier) {
            savedQuote.supplier = supplier;
        }
        ...
        return server_1.NextResponse.json({ error: 'Failed to process supplier quote OCR' }, { status: 500 });
    }
}
...
async function GET(request) {
    try {
        ...
        const client = await (0, mongodb_client_1.getMongoClient)();
        const db = client.db(DB_NAME);
        const filter = { isGeneratedByAI: true };
        if (supplierId) {
            filter.supplierId = new mongodb_1.ObjectId(supplierId);
        }
        if (dealId) {
            filter.dealId = new mongodb_1.ObjectId(dealId);
        }
