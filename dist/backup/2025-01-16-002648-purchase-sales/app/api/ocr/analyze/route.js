...
const mastra_orchestrator_1 = __importDefault(require("@/mastra-orchestrator"));
const mongodb_client_1 = require("@/lib/mongodb-client");
const ocr_date_extractor_1 = require("@/lib/ocr-date-extractor");
// 金額文字列から数値を抽出する関数
...
async function POST(request) {
    try {
        ...
        // Mastraエージェントの確認
        const availableAgents = mastra_orchestrator_1.default.getAvailableAgents();
        ...
        const companyId = formData.get('companyId') || 'default';
        ...
        const { getFormRecognizerService } = await Promise.resolve().then(() => __importStar(require('@/lib/azure-form-recognizer')));
        ...
        // GridFSにファイルを保存
        const sourceFileId = await formRecognizer.saveToGridFS(buffer, file.name, { companyId: companyId, uploadedAt: new Date().toISOString() });
        // OCR結果をMongoDBに保存
        const ocrResult = {
        ...
        try {
            ...
            const dateExtraction = ocr_date_extractor_1.OCRDateExtractor.extractDate(extractedTextForDate);
            ...
            try {
                ...
                const prediction = await categoryAI.predictAccountCategory(ocrResultForAI, companyId === 'default' ? '11111111-1111-1111-1111-111111111111' : companyId);
                ...
                try {
                    ...
                    const journalEntry = await ocrProcessor.createJournalEntry(ocrResultForJournal, companyId === 'default' ? '11111111-1111-1111-1111-111111111111' : companyId);
        ...
        try {
            ...
            const extractedText = JSON.stringify(analysisResult.fields || {});
            const responseDateExtraction = ocr_date_extractor_1.OCRDateExtractor.extractDate(extractedText);
