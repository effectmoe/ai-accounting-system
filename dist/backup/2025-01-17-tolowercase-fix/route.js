...
async function POST(request) {
    try {
        ...
        const documentType = formData.get('documentType') || 'receipt';
        ...
        if (azureConfigured) {
            const { getFormRecognizerService } = await Promise.resolve().then(() => __importStar(require('@/lib/azure-form-recognizer')));
            const formRecognizer = getFormRecognizerService();
            sourceFileId = await formRecognizer.saveToGridFS(buffer, file.name, { companyId: companyId, uploadedAt: new Date().toISOString() });
        }
        else {
            // モックの場合はランダムなIDを生成
            sourceFileId = new mongodb_1.ObjectId().toString();
