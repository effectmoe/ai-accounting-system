...
class OCRRuleBasedOrchestrator {
    ...
    static orchestrateFromTables(ocrResult) {
        ...
        const data = {};
        try {
            logger_1.logger.debug('[RuleBasedOrchestrator] === オーケストレータ開始 ===');
            // デバッグ: 受信したOCRデータの構造を確認
            logger_1.logger.debug('[RuleBasedOrchestrator] 受信したOCRデータ:', {
            ...
            if (!ocrResult) {
                ...
                logger_1.logger.error('[RuleBasedOrchestrator] ' + error);
            ...
            // デバッグ: フィールドの内容を確認
            logger_1.logger.debug('[RuleBasedOrchestrator] fieldsの内容:', {
            ...
            // デバッグ: 御中判定前の状態
            logger_1.logger.debug('[RuleBasedOrchestrator] 御中判定前:', {
                vendorName,
                customerName,
            ...
            logger_1.logger.debug('[RuleBasedOrchestrator] 会社名・顧客名の判定を開始');
            ...
            logger_1.logger.debug('[RuleBasedOrchestrator] 全フィールド値:', allFieldValues);
            // 1. 「御中」で判別（確実な判定）
            for (const fieldValue of allFieldValues) {
                if (fieldValue.includes('御中')) {
                    identifiedCustomer = fieldValue;
                    logger_1.logger.debug('[RuleBasedOrchestrator] 「御中」で顧客を特定:', fieldValue);
                    break;
                }
            }
            // 2. 残りの会社名から仕入先を特定
            if (identifiedCustomer) {
                ...
                for (const fieldValue of allFieldValues) {
                    if (fieldValue !== identifiedCustomer && this.isCompanyName(fieldValue)) {
                        ...
                        logger_1.logger.debug('[RuleBasedOrchestrator] 残りから仕入先を特定:', fieldValue);
                        break;
                    }
                }
            }
                ...
                for (const fieldValue of allFieldValues) {
                    if (this.isCompanyName(fieldValue)) {
                        identifiedVendor = fieldValue;
                        logger_1.logger.debug('[RuleBasedOrchestrator] 会社形態で仕入先を特定:', fieldValue);
                        break;
                    }
                }
                // 顧客は残りから選択
                for (const fieldValue of allFieldValues) {
                    if (fieldValue !== identifiedVendor) {
                        identifiedCustomer = fieldValue;
                        logger_1.logger.debug('[RuleBasedOrchestrator] 残りから顧客を特定:', fieldValue);
                        break;
                    }
                }
            }
            ...
            logger_1.logger.debug('[RuleBasedOrchestrator] 判定完了:', {
            ...
            if (ocrResult.tables && ocrResult.tables.length > 0) {
                logger_1.logger.debug('[RuleBasedOrchestrator] テーブルから商品を抽出');
            ...
            if (data.items.length === 0 && ocrResult.pages && ocrResult.pages.length > 0) {
                logger_1.logger.debug('[RuleBasedOrchestrator] ページから商品を抽出');
            ...
            if ((!data.vendor || data.vendor.name === '不明') && ocrResult.pages && ocrResult.pages.length > 0) {
                logger_1.logger.debug('[RuleBasedOrchestrator] pagesから仕入先情報を探索');
                // 右側にある会社名を優先的に収集
                        ...
                        for (const line of page.lines) {
                            if (line.content) {
                                ...
                                if (isCompanyName) {
                                    ...
                                    logger_1.logger.debug('[RuleBasedOrchestrator] 会社名候補を発見:', content);
                                    ...
                                    if (line.boundingBox && line.boundingBox.length > 0) {
                                        ...
                                        logger_1.logger.debug('[RuleBasedOrchestrator] boundingBox情報:', {
                                            content: content,
                                        ...
                                        if (xPosition > pageWidth * 0.5) {
                                            ...
                                            logger_1.logger.debug('[RuleBasedOrchestrator] 右側の会社名を発見:', content, 'at x:', xPosition);
                                        }
                                        else {
                                            logger_1.logger.debug('[RuleBasedOrchestrator] 左側の会社名:', content, 'at x:', xPosition);
                                        }
                                    }
                                    else {
                                        logger_1.logger.debug('[RuleBasedOrchestrator] boundingBox情報なし:', content);
                                    }
                                }
                            }
                        }
                ...
                if (selectedCompany) {
                    ...
                    logger_1.logger.debug('[RuleBasedOrchestrator] 仕入先名を決定:', selectedCompany.content);
                }
                // 住所と電話番号の抽出（元のロジック）
                for (const page of ocrResult.pages) {
                    if (page.lines) {
                        for (const line of page.lines) {
                            if (line.content) {
                                ...
                                if ((content.includes('県') || content.includes('都') || content.includes('府') ||
                                    ...
                                    logger_1.logger.debug('[RuleBasedOrchestrator] pagesから住所を抽出:', content);
                                }
                                // 電話番号パターン
                                ...
                                if (phoneMatch && !data.vendor?.phone) {
                                    ...
                                    logger_1.logger.debug('[RuleBasedOrchestrator] pagesから電話番号を抽出:', phoneMatch[0]);
                                }
                            }
                        }
                    }
            ...
            if (ocrResult.pages && ocrResult.pages.length > 0) {
                logger_1.logger.debug('[RuleBasedOrchestrator] pagesから件名を探索');
                for (const page of ocrResult.pages) {
                    if (page.lines) {
                        for (let i = 0; i < page.lines.length; i++) {
                            const line = page.lines[i];
                            if (line.content && line.content.includes('件名')) {
                                logger_1.logger.debug('[RuleBasedOrchestrator] 「件名」ラベルを発見:', line.content);
                                // 同じ行の「件名」の後の部分を確認
                                const sameLineMatch = line.content.match(/件名[:\s：]*(.+)/);
                                if (sameLineMatch && sameLineMatch[1].trim()) {
                                    data.subject = sameLineMatch[1].trim();
                                    logger_1.logger.debug('[RuleBasedOrchestrator] 同じ行から件名を抽出:', data.subject);
                                    break;
                                }
                                // 次の行が実際の件名
                                if (i + 1 < page.lines.length) {
                                    ...
                                    if (nextLine && !nextLine.includes('件名')) {
                                        ...
                                        logger_1.logger.debug('[RuleBasedOrchestrator] 次の行から件名を抽出:', data.subject);
                                        break;
                                    }
                                }
                            }
            ...
            if (data.subject && data.subject.includes('様分')) {
                logger_1.logger.debug('[RuleBasedOrchestrator] 件名が顧客向けテキストになっています。正しい件名を再検索。');
            ...
            logger_1.logger.debug('[RuleBasedOrchestrator] 最終結果:', {
                hasValidData,
            ...
            logger_1.logger.debug('[RuleBasedOrchestrator] === オーケストレータ完了 ===');
            return {
            ...
            logger_1.logger.error('[RuleBasedOrchestrator] エラー:', error);
            return {
                success: false,
    ...
    static extractItemsFromTable(table) {
        ...
        for (let rowIndex = 1; rowIndex < rows.length; rowIndex++) {
            ...
            if (productName && !this.isHeaderText(productName) && (amount > 0 || unitPrice > 0)) {
                ...
                logger_1.logger.debug(`[RuleBasedOrchestrator] 商品抽出: ${productName}, 数量: ${quantity}, 単価: ${unitPrice}, 金額: ${amount}`);
            }
        }
        return items;
    }
