...
import { accountingTools } from './tools/eecf28a3-a59e-4209-9984-caca4c988c80.mjs';
import { customerTools } from './tools/cec6a8ac-f894-4d67-9a87-1f2742077050.mjs';
...
const mastraDeploymentAgent = new Agent({
...
const mastraJapanTaxAgent = new Agent({
...
const mastraOcrAgent = new Agent({
...
const mastraProblemSolvingAgent = new Agent({
...
const mastraProductAgent = new Agent({
...
const mastraRefactorAgent = new Agent({
...
const mastraConstructionAgent = new Agent({
...
const accountingWorkflow = createWorkflow({
    ...
    },
    {
      ...
      input: (workflowInput, previousResults) => ({
        ...
        customerData: {
          ...
          lastInvoiceDate: (/* @__PURE__ */ new Date()).toISOString(),
  ...
  execute: async ({ input, agents, context }) => {
    try {
      logger$1.debug("[Accounting Workflow] Starting workflow:", input.workflowType);
      const results = {};
      for (const step of accountingWorkflow.steps) {
        if (step.condition && !step.condition(input)) {
          logger$1.debug(`[Accounting Workflow] Skipping step: ${step.name}`);
          continue;
        }
        logger$1.debug(`[Accounting Workflow] Executing step: ${step.name}`);
      ...
      logger$1.error("[Accounting Workflow] Error:", error);
      if (accountingWorkflow.errorHandling.compensation) {
        logger$1.debug("[Accounting Workflow] Initiating rollback...");
      }
      throw error;
    }
  }
...
const complianceWorkflow = createWorkflow({
  ...
  steps: [
    ...
    },
    {
      ...
      input: (workflowInput) => ({
        ...
        complianceCheckOptions: {
          ...
          targetPeriod: workflowInput.taxReturnData?.fiscalYear || (/* @__PURE__ */ new Date()).getFullYear().toString()
        }
      })
    },
    {
    ...
    },
    {
      ...
      input: (workflowInput, previousResults) => {
        ...
        const totalRevenue = financialData.filter((t) => t.transaction_type === "income").reduce((sum, t) => sum + t.amount, 0);
        const totalExpenses = financialData.filter((t) => t.transaction_type === "expense").reduce((sum, t) => sum + t.amount, 0);
  ...
  execute: async ({ input, agents, context }) => {
    try {
      logger$1.debug("[Compliance Workflow] Starting compliance check:", input.complianceType);
      const results = {};
      for (const step of complianceWorkflow.steps) {
        if (step.condition && !step.condition(input, results)) {
          logger$1.debug(`[Compliance Workflow] Skipping step: ${step.name}`);
          continue;
        }
        logger$1.debug(`[Compliance Workflow] Executing step: ${step.name}`);
        const stepInput = step.input(input, results);
        const stepResult = await agents[step.agent].execute({
          input: stepInput,
        ...
        results[step.id] = stepResult;
        if (!stepResult.success) {
          logger$1.warn(`[Compliance Workflow] Step ${step.name} completed with issues:`, stepResult.error);
        }
      }
      ...
      logger$1.error("[Compliance Workflow] Error:", error);
      throw error;
    }
  }
});
...
const invoiceProcessingWorkflow = createWorkflow({
      ...
      input: (workflowInput, previousResults) => {
        if (workflowInput.processingType === "incoming_invoice") {
          ...
          return {
            ...
            consumptionTax: {
              period: {
                startDate: invoiceData?.date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
                endDate: invoiceData?.date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0]
              },
              salesData: {
                standardRateSales: 0,
                reducedRateSales: 0,
        ...
        if (workflowInput.processingType === "incoming_invoice") {
          ...
          return {
            ...
            entryData: {
              ...
              date: invoiceData?.date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
              taxData,
  ...
  execute: async ({ input, agents, context }) => {
    try {
      logger$1.debug("[Invoice Processing Workflow] Starting processing:", input.processingType);
      const results = {};
      ...
      for (const step of invoiceProcessingWorkflow.steps) {
        if (step.condition && !step.condition(input, results)) {
          logger$1.debug(`[Invoice Processing Workflow] Skipping step: ${step.name}`);
          continue;
        }
        logger$1.debug(`[Invoice Processing Workflow] Executing step: ${step.name}`);
        const stepInput = step.input(input, results);
        const stepResult = await agents[step.agent].execute({
          input: stepInput,
        ...
        results[step.id] = stepResult;
        if (!stepResult.success) {
          logger$1.warn(`[Invoice Processing Workflow] Step ${step.name} completed with issues:`, stepResult.error);
        }
      }
      return {
        success: true,
      ...
      logger$1.error("[Invoice Processing Workflow] Error:", error);
      throw error;
    }
  }
});
...
const deploymentWorkflow = new Workflow({
  ...
  hooks: {
    beforeWorkflow: async (context) => {
      logger$1.debug("[Deployment Workflow] Starting deployment workflow...");
    },
    afterWorkflow: async (context, result) => {
      logger$1.debug("[Deployment Workflow] Workflow completed");
      if (result.success) {
        logger$1.debug(`[Deployment Workflow] Successfully deployed to: ${result.url}`);
      } else {
        logger$1.error(`[Deployment Workflow] Deployment failed: ${result.error}`);
      }
    },
    onError: async (error, context) => {
      logger$1.error(`[Deployment Workflow] Error in step ${context.currentStep}:`, error);
    }
  }
});
