...
/**
 * システム全体のMongoDB対応ヘルスチェックスクリプト
 * すべてのエージェント、ワークフロー、設定のMongoDB対応状況を検証
 */

import * as fs from 'fs';
import * as path from 'path';
import { DatabaseService, Collections, getDatabase, getClientPromise } from '../lib/mongodb-client.js';
...
class SystemHealthChecker {
  ...
  async runAllChecks(): Promise<void> {
    try {
      ...
      // 3. エージェントMongoDB対応チェック
      await this.checkAgentMongoDBCompliance();
      
      // 4. ワークフローMongoDB対応チェック
      await this.checkWorkflowMongoDBCompliance();
      
      // 5. mastra.config.ts チェック
      this.checkMastraConfig();
      
      // 6. データベースクライアント設定チェック
  ...
  private async checkMongoDBConnection(): Promise<void> {
    ...
    try {
      ...
      result.status = 'PASS';
      result.details.push('✅ MongoDB接続成功');
      
      // コレクション確認
      const collections = await db.listCollections().toArray();
      result.details.push(`📊 利用可能コレクション数: ${collections.length}`);
      
      // Collections定数と実際のコレクションの照合
      const definedCollections = Object.values(Collections);
      const existingCollections = collections.map((c: any) => c.name);
      
      for (const collection of definedCollections) {
        if (existingCollections.includes(collection)) {
          result.details.push(`✅ コレクション存在確認: ${collection}`);
        } else {
          result.details.push(`⚠️  コレクション未存在: ${collection}`);
        }
      }
      
    } catch (error) {
      result.errors.push(`MongoDB接続エラー: ${error}`);
    }
   ...
   */
  private async checkAgentMongoDBCompliance(): Promise<void> {
    ...
    for (const agentFile of agentFiles) {
      ...
      try {
        ...
        result.errors.push(`ファイル読み込みエラー: ${error}`);
        result.status = 'FAIL';
      }
   ...
   */
  private async checkWorkflowMongoDBCompliance(): Promise<void> {
      ...
      try {
        ...
        const checks = [
          ...
          },
          {
            ...
            pattern: /operation:\s*['"](?:insert|select|update|delete)['"]|table:\s*['"][^'"]+['"]/,
            required: false,
            shouldNotExist: true
          },
          {
   ...
   */
  private checkMastraConfig(): void {
    const result: HealthCheckResult = {
    ...
    try {
      ...
      result.errors.push(`設定ファイル読み込みエラー: ${error}`);
      result.status = 'FAIL';
    }
   ...
   * データベースクライアント設定チェック
   */
  private checkDatabaseClientConfig(): void {
    const result: HealthCheckResult = {
    ...
    try {
      const clientPath = path.join(__dirname, '../src/lib/mongodb-client.ts');
      ...
      result.errors.push(`クライアント設定読み込みエラー: ${error}`);
      result.status = 'FAIL';
    }
   ...
   * Supabase残存参照チェック
   */
  private checkSupabaseReferences(): void {
    const result: HealthCheckResult = {
    ...
    for (const dir of searchDirs) {
      try {
        ...
        result.errors.push(`ディレクトリスキャンエラー ${dir}: ${error}`);
      }
    }
   ...
   * Azure設定チェック
   */
  private checkAzureConfiguration(): void {
    const result: HealthCheckResult = {
    ...
    try {
      ...
      result.errors.push(`OCRエージェント読み込みエラー: ${error}`);
      result.status = 'FAIL';
    }
