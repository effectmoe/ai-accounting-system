const { DatabaseService, Collections } = require('../lib/mongodb-client');
...
async function debugParkingFields() {
  ...
  try {
    ...
    const ocrResults = await db.find(Collections.OCR_RESULTS, {
      receiptType: 'parking'
    }, {
      limit: 5,
      sort: { createdAt: -1 }
    ...
    if (documents.length > 0) {
      ...
      console.log(Object.keys(sample).filter(key => 
    ...
    if (testId) {
      ...
      try {
        const doc = await db.findById(Collections.DOCUMENTS, testId);
        if (doc) {
          console.log('\nDocument found:');
          console.log({
            id: doc._id,
    ...
    const recentDocs = await db.find(Collections.DOCUMENTS, {}, {
      limit: 3,
      sort: { createdAt: -1 }
    });
