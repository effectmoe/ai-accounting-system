...
async function debugParkingDataFlow(documentId?: string) {
  ...
  try {
    ...
    for (const doc of documents) {
      ...
      if (doc.ocrResultId || doc.ocr_result_id) {
        ...
        const ocrResult = await ocrResultsCollection.findOne({ _id: new ObjectId(ocrId) });
    ...
    // OCR処理での問題チェック
    const hasOcrParkingData = debugLog.some(log => 
      log.step === 'OCR Results Collection' && Object.keys(log.parkingFields).length > 0
    );
    
    const hasDocumentParkingData = debugLog.some(log => 
      log.step === 'Document Fields' && Object.keys(log.parkingFields).length > 0
    );
