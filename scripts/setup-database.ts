...
import { DatabaseSetupAgent } from '../src/agents/database-setup-agent';
...
async function setupDatabase() {
  ...
  try {
    ...
    const mcpServer = spawn('tsx', [
      join(process.cwd(), 'src/mcp-servers/database-mcp-server.ts')
    ], {
      ...
      mcpServers: [
        {
          ...
          transport: {
            type: 'stdio',
            command: 'tsx',
            args: [join(process.cwd(), 'src/mcp-servers/database-mcp-server.ts')],
            env: {
              SUPABASE_URL: process.env.NEXT_PUBLIC_SUPABASE_URL,
              SUPABASE_SERVICE_ROLE_KEY: process.env.SUPABASE_SERVICE_ROLE_KEY,
            },
    ...
    const agent = mastra.getAgent('database-setup-agent') as DatabaseSetupAgent;
