...
export class OCRProcessor {
  ...
  async processImageFile(imageFile: File): Promise<OCRResult> {
    ...
    if (typeof window === 'undefined' && isGoogleCloudConfigured && process.env.ENABLE_OCR === 'true') {
      ...
      try {
        const { GoogleCloudOCRProcessor } = await import('./ocr-processor-google');
        const googleOCR = new GoogleCloudOCRProcessor();
        
        // FileをBufferに変換
        const arrayBuffer = await imageFile.arrayBuffer();
        const buffer = Buffer.from(arrayBuffer);
        
        const result = await googleOCR.processReceiptImage(buffer);
        await googleOCR.close();
