...
export class FirecrawlClient {
   ...
   * 単一ページのスクレイピング
   */
  async scrape(request: FirecrawlScrapeRequest): Promise<ScrapeResult> {
    if (!this.apiKey) {
      return {
        success: false,
   ...
   * 複数ページのクロール
   */
  async crawl(request: FirecrawlCrawlRequest): Promise<CrawlResult> {
    if (!this.apiKey) {
      return {
        success: false,
        error: 'Firecrawl API key not configured',
   ...
   */
  async search(request: FirecrawlSearchRequest): Promise<SearchResult> {
    if (!this.apiKey) {
      return {
        success: false,
        error: 'Firecrawl API key not configured',
   ...
   * 競合他社サイトの分析
   */
  async analyzeCompetitorSite(url: string): Promise<{
    success: boolean;
    analysis?: {
      content: string;
      title: string;
   ...
   * サイトマップベースのクロール
   */
  async crawlSitemap(sitemapUrl: string, options?: {
    limit?: number;
