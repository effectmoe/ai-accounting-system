  ...
  try {
    ...
    for (const simpleFaq of simpleFaqs) {
      try {
        ...
        // 元のFAQを非アクティブに更新
        await simpleFaqCollection.updateOne(
          { _id: simpleFaq._id },
          { 
            $set: { 
              status: 'migrated',
...
// カテゴリをコンテンツタイプにマッピング
function mapCategoryToContentType(category?: string): 'tax' | 'accounting' | 'invoice' | 'compliance' | 'procedure' | 'general' {
  if (!category) return 'general';
  
  const mapping: Record<string, 'tax' | 'accounting' | 'invoice' | 'compliance' | 'procedure' | 'general'> = {
    'tax': 'tax',
    'tax-accounting': 'tax',
...
// 回答から税法を抽出
function extractTaxLaws(answer: string): string[] {
  const taxLaws: string[] = [];
